...
export class HostCredentialRateLimitExceeded extends Error {
    retryAfterSeconds;
    code = 'host_credential_rate_limited';
...
export class HostCredentialVerifierBusy extends Error {
    code = 'host_credential_verifier_busy';
    constructor() {
...
export class HostCredentialAdmission {
    ...
    constructor(options = {}) {
        ...
        if (!Number.isSafeInteger(slots) || slots < 1_024 || slots > 1_048_576 || slots & (slots - 1)) {
            throw new Error('host credential admission slots must be a power of two from 1024 to 1048576');
        }
        if (!Number.isSafeInteger(requestsPerSecond) ||
            requestsPerSecond < 1 ||
            requestsPerSecond > 1_000) {
            throw new Error('host credential requests per second must be from 1 to 1000');
        }
        if (!Number.isSafeInteger(maximumConcurrentVerifications) ||
    ...
    async verify(request, hostId, operation) {
        ...
        if (slot.count > this.#requestsPerSecond) {
            throw new HostCredentialRateLimitExceeded(Math.max(1, Math.ceil(((window + 1) * 1_000 - now) / 1_000)));
        }
...
//# sourceMappingURL=host-admission.js.map