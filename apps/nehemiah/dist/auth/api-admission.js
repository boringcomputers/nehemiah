...
export class ApiAdmissionUnavailable extends Error {
    code = 'api_admission_unavailable';
    constructor() {
        super('API admission could not be verified.');
    }
}
const ipv4Network = (address) => {
 ...
 */
export const coarseClientIdentity = (raw) => {
    const address = (raw ?? '').trim().replace(/^::ffff:(?=\d+\.\d+\.\d+\.\d+$)/i, '');
    if (isIP(address) === 4)
        return `ipv4:${ipv4Network(address) ?? 'invalid'}`;
    if (isIP(address) === 6) {
        const bytes = ipv6Bytes(address);
        if (bytes)
            return `ipv6:${Buffer.from(bytes.slice(0, 7)).toString('hex')}/56`;
    }
    return 'unknown';
};
const slotFor = (scope, identifier) => createHash('sha256').update(scope).update('\0').update(identifier).digest().readUInt32BE(0) &
    (bucketSlots - 1);
export class PostgresApiAdmission {
    database;
    config;
    ...
    async preAuthenticate(request, token) {
    ...
    async consume(buckets) {
        if (!this.config.enabled)
            return;
        const ordered = [...buckets].sort((left, right) => left.scope.localeCompare(right.scope) || left.identifier.localeCompare(right.identifier));
        try {
            const result = await this.database.query(`WITH requested(scope, bucket_slot, request_limit) AS (
            ...
            if (!row?.allowed) {
                ...
                throw new ApiRateLimitExceeded(Number.isSafeInteger(retryAfter) && retryAfter > 0
...
//# sourceMappingURL=api-admission.js.map