...
const finiteNumber = (env, name, fallback) => {
    const raw = env[name];
    if (raw === undefined || raw === '')
        return fallback;
    const value = Number(raw);
...
const requireStrongProductionSecret = (production, name, value) => {
...
const validateDeviceCodePepper = (value) => {
    const encoded = Redacted.value(value);
    const decoded = Buffer.from(encoded, 'base64');
...
const siteBoundarySuffix = (domain) => domain.split('.').slice(-2).join('.');
const requiredStorageValue = (env, name) => {
...
const loadObjectStorage = (env) => {
    ...
    if (!variables.some((name) => env[name] !== undefined))
        return undefined;
...
const loadTemplateTransfers = (env, objectStorage, production) => {
    const configured = [
    ...
    if (!enabled) {
        if (env.NEHEMIAH_TEMPLATE_REPLICATION_INTERVAL_MS !== undefined ||
    ...
    if (production) {
        throw new ConfigError({
            variable: 'NEHEMIAH_TEMPLATE_TRANSFERS_ENABLED',
            message: 'managed custom-template publication is disabled in production until aggregate tenant and host-cache quotas plus durable eviction are enforced'
        });
    }
    if (!objectStorage) {
        throw new ConfigError({
...
const loadVolumeBroker = (env, objectStorage, production) => {
    const variables = ['NEHEMIAH_VOLUME_BROKER_URL', 'NEHEMIAH_VOLUME_BROKER_SECRET'];
    if (!variables.some((name) => env[name] !== undefined))
        return undefined;
    if (production) {
        throw new ConfigError({
            variable: 'NEHEMIAH_VOLUME_BROKER_URL',
            message: 'managed volume transfers are disabled in production until durable volume/revision counts and global transfer admission are enforced'
        });
    }
    ...
    if (!/^[A-Za-z0-9+/]{43}=$/.test(encodedSecret) ||
...
const loadTelemetry = (env) => {
    ...
    const configured = env.NEHEMIAH_OTEL_ENABLED !== undefined || tuning.some((name) => env[name] !== undefined);
    if (!configured)
        return undefined;
    ...
    if (authorization.length < 16 ||
        ...
        throw new ConfigError({
            ...
            message: 'must be a 16-4096 character HTTP authorization value without whitespace in its credential'
        });
    }
...
const loadApiRateLimit = (env, production) => {
    ...
    const boundedLimit = (name, fallback) => {
...
export const loadConfig = (env = process.env) => Effect.try({
    try: () => {
        ...
        const databaseUrl = secret(env, 'DATABASE_URL', 'postgres://postgres:postgres@127.0.0.1:5432/nehemiah', production);
        if (production) {
            const issue = productionDatabaseUrlIssue(Redacted.value(databaseUrl));
            if (issue)
                throw new ConfigError({ variable: 'DATABASE_URL', message: issue });
        }
        const hostCredentialKey = secret(env, 'NEHEMIAH_HOST_CREDENTIAL_KEY', 'MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=', production);
        validateCredentialKey(hostCredentialKey);
        const gatewayToken = secret(env, 'NEHEMIAH_GATEWAY_TOKEN', 'dev-gateway-token', production);
        const gatewaySecret = secret(env, 'NEHEMIAH_GATEWAY_SECRET', 'dev-gateway-secret', production);
        const deviceCodePepper = secret(env, 'NEHEMIAH_DEVICE_CODE_PEPPER', 'bmVoZW1pYWgtZGV2aWNlLWNvZGUtcGVwcGVyLWRldiE=', production);
        ...
        const platformSecrets = [
            ...
            ...(volumeBroker
                ? [['NEHEMIAH_VOLUME_BROKER_SECRET', Redacted.value(volumeBroker.secret)]]
                : []),
            ...(telemetry
                ? [
                    [
                        'NEHEMIAH_OTEL_AUTHORIZATION',
                        Redacted.value(telemetry.authorization).slice(Redacted.value(telemetry.authorization).indexOf(' ') + 1)
                    ]
                ]
                : [])
        ];
        if (production) {
            ...
            for (const [variable, value] of platformSecrets) {
                if (seenSecrets.has(value)) {
                    throw new ConfigError({
                        variable,
                        message: 'host encryption, gateway service, device-code lookup, telemetry, and volume capability secrets must be distinct'
                    });
                }
        ...
        if ((production && !previewBaseDomain) ||
        ...
        const deviceVerificationValue = env.NEHEMIAH_DEVICE_VERIFICATION_URL ??
            (trustedSiteDomain
                ? `https://${trustedSiteDomain}/dashboard/device`
                : 'http://localhost:5173/dashboard/device');
        let deviceVerificationUrl;
        try {
        ...
        if (!['http:', 'https:'].includes(deviceVerificationUrl.protocol) ||
        ...
        const hostCidrs = (env.NEHEMIAH_HOST_CIDRS ??
            (production
