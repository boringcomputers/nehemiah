...
const positiveBatch = (value, maximum, label) => {
    if (!Number.isSafeInteger(value) || value < 1 || value > maximum) {
        throw new Error(`${label} must be from 1 to ${maximum}`);
    }
};
const reapTransaction = async (client, options) => {
    ...
    try {
        ...
        const access = await client.query(`WITH terminal AS (
			 ...
			 RETURNING token.id`, [options.accessBatch]);
        ...
        const familyRows = await client.query(`SELECT id, device_authorization_id
			 FROM device_refresh_families
			 WHERE COALESCE(revoked_at, expires_at)
			       <= statement_timestamp() - interval '24 hours'
			 ORDER BY COALESCE(revoked_at, expires_at), id
			 LIMIT $1 FOR UPDATE SKIP LOCKED`, [options.familyBatch]);
        ...
        if (familyIds.length > 0) {
            const familyAccess = await client.query('DELETE FROM device_access_tokens WHERE family_id = ANY($1::uuid[])', [familyIds]);
            familyAccessCount = familyAccess.rowCount ?? 0;
            const refresh = await client.query('DELETE FROM device_refresh_tokens WHERE family_id = ANY($1::uuid[])', [familyIds]);
            refreshCount = refresh.rowCount ?? 0;
            const families = await client.query('DELETE FROM device_refresh_families WHERE id = ANY($1::uuid[])', [familyIds]);
            familyCount = families.rowCount ?? 0;
            const authorizations = await client.query(`DELETE FROM device_authorizations AS device_auth
        ...
        const authorizations = await client.query(`WITH terminal AS (
			 ...
			 RETURNING device_auth.id`, [options.authorizationBatch]);
        ...
        return {
            ...
            authorizations: familyAuthorizationCount + (authorizations.rowCount ?? authorizations.rows.length)
        };
    }
    catch (error) {
        try {
 ...
 */
export const reapExpiredDeviceAuthorizations = async (database, options = {}) => {
    ...
    return ((await database.withAdvisoryLock(deviceRetentionAdvisoryLock, (client) => reapTransaction(client, { accessBatch, familyBatch, authorizationBatch }))) ?? emptyReport(true));
};
//# sourceMappingURL=reap-expired-device-authorizations.js.map