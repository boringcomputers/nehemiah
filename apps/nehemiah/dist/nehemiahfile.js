...
export const parseNehemiahfile = (source) => {
    if (Buffer.byteLength(source) > 256 * 1024)
        throw new Error('Nehemiahfile exceeds 256 KiB');
    const root = object(parse(source), 'Nehemiahfile');
    if (root.version !== 1)
        throw new Error('Nehemiahfile version must be 1');
    ...
    const networkInput = root.network === undefined ? { mode: 'off' } : object(root.network, 'network');
    if (networkInput.mode !== undefined &&
    ...
    const ports = root.ports === undefined ? [] : root.ports;
    ...
    return {
        ...
        readiness: readinessInput
            ? {
                tcp_port: readinessInput.tcp_port === undefined
                    ? undefined
                    : positive(readinessInput.tcp_port, 'readiness.tcp_port'),
                http_path: typeof readinessInput.http_path === 'string' ? readinessInput.http_path : undefined
            }
            : undefined
    };
};
//# sourceMappingURL=nehemiahfile.js.map