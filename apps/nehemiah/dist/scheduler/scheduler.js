...
export class Scheduler {
    ...
    async reserve(request) {
        ...
        return this.database.transaction(async (client) => {
            ...
            await client.query(`UPDATE hosts SET
				 ...
				 updated_at = now()
				 WHERE id = $1`, [host.id, request.resources.vcpus, request.resources.memoryMb, request.resources.diskMb]);
            if (request.machineId) {
                const assigned = await client.query(`UPDATE machines SET host_id = $2, runtime_cohort_id = $3,
					 ...
					 RETURNING organization_id`, [request.machineId, host.id, host.runtime_cohort_id, host.source_sha256]);
                const machine = assigned.rows[0];
                if (!machine)
                    throw new Error('machine could not be assigned atomically');
                await client.query(`INSERT INTO machine_events
					 ...
					 ON CONFLICT (event_key) DO NOTHING`, [`${request.machineId}:assigned`, request.machineId, machine.organization_id]);
            }
            return {
                hostId: host.id,
    ...
    async release(hostId, resources, machineId) {
        await this.database.transaction(async (client) => {
            if (machineId) {
                const claimed = await client.query(`UPDATE machines SET reservation_released_at = now()
					 WHERE id = $1 AND reservation_released_at IS NULL`, [machineId]);
                if (!claimed.rowCount)
                    return;
            }
            await client.query(`UPDATE hosts SET
    ...
    async enforceQuota(client, request) {
        ...
        const organizationResult = await client.query(`SELECT o.max_machines, o.max_vcpus, o.max_memory_mb, o.max_disk_mb
			 FROM organizations o
			 WHERE o.id = $1 FOR UPDATE OF o`, [request.organizationId]);
        const organizationLimits = organizationResult.rows[0];
        if (!organizationLimits)
            throw new QuotaExceeded('Organization does not exist.');
        let candidateSeconds = 0n;
        if (request.machineId) {
            const candidate = await client.query(`SELECT CEIL(GREATEST(
				   EXTRACT(EPOCH FROM (expires_at - statement_timestamp())), 0
				 ))::numeric::text AS remaining_seconds
				 FROM machines
				 WHERE id = $1 AND organization_id = $2 AND project_id = $3
				   AND state IN ('requested', 'placing')`, [request.machineId, request.organizationId, request.projectId]);
            if (candidate.rows[0])
                candidateSeconds = BigInt(candidate.rows[0].remaining_seconds);
        }
        ...
        const limitsResult = await client.query(`SELECT max_machines, max_vcpus, max_memory_mb, max_disk_mb FROM projects
        ...
        const usageResult = await client.query(`SELECT count(*) AS machines, COALESCE(sum(vcpus), 0) AS vcpus,
			 ...
			 AND state NOT IN ('requested', 'placing', 'stopped', 'failed', 'lost')`, [request.projectId]);
        const usage = usageResult.rows[0];
        const organizationUsageResult = await client.query(`SELECT count(*) AS machines, COALESCE(sum(vcpus), 0) AS vcpus,
			        COALESCE(sum(memory_mb), 0) AS memory_mb,
			        COALESCE(sum(disk_mb), 0) AS disk_mb
			 FROM machines WHERE organization_id = $1
			 AND state NOT IN ('requested', 'placing', 'stopped', 'failed', 'lost')`, [request.organizationId]);
        const organizationUsage = organizationUsageResult.rows[0];
        if (Number(organizationUsage.machines) + 1 > organizationLimits.max_machines ||
