...
import { DeleteObjectCommand, DeleteObjectsCommand, GetObjectCommand, HeadObjectCommand, ListObjectVersionsCommand, ListObjectsV2Command, PutObjectCommand, S3Client } from '@aws-sdk/client-s3';
...
const validBrokerUrl = (value) => {
    ...
    try {
        ...
        throw new S3VolumeObjectStoreContractError('Volume broker public URL must be an origin-only HTTPS URL.');
    }
...
const validStorageConfig = (config) => {
...
const capabilityKey = (encoded) => {
    if (!/^[A-Za-z0-9+/]{43}=$/.test(encoded)) {
        throw new S3VolumeObjectStoreContractError('Volume broker secret must be a base64-encoded 32-byte key.');
    }
...
const volumePrefix = (organizationId, projectId, objectPrefix) => {
    ...
    if (!objectPrefix.endsWith('/') ||
        ...
        throw new S3VolumeObjectStoreContractError('Volume object prefix is outside the managed tenant namespace.');
    }
    return objectPrefix;
};
const revisionPrefix = (objectPrefix) => `${objectPrefix}revisions/`;
const revisionKey = (objectPrefix, reservationId) => {
...
const validRevisionKey = (objectPrefix, key) => new RegExp(`^${revisionPrefix(objectPrefix).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}${uuid}\\.bin$`, 'i').test(key);
const checksumHex = (encoded) => {
    if (!encoded || !/^[A-Za-z0-9+/]{43}=$/.test(encoded)) {
        throw new S3VolumeObjectStoreContractError('Stored volume revision is missing a full SHA-256 checksum.');
    }
    const digest = Buffer.from(encoded, 'base64');
    if (digest.byteLength !== 32 || digest.toString('base64') !== encoded) {
        throw new S3VolumeObjectStoreContractError('Stored volume revision has an invalid SHA-256 checksum.');
    }
...
const isPreconditionFailed = (error) => typeof error === 'object' &&
    error !== null &&
    (('name' in error &&
...
const exactHeader = (request, name) => {
...
const jsonProblem = (response, status, code, detail, close = false) => {
...
const drainAndHash = async (request, expectedBytes, maximumBytes, abortSignal) => {
    let bytes = 0;
    ...
    await pipeline(request, meter, new Writable({
    ...
    if (bytes !== expectedBytes) {
        throw new S3VolumeObjectStoreContractError('Volume upload length did not match Content-Length.');
    }
 ...
 */
export class S3VolumeObjectStore {
    deletionMode = 'at-retention-boundary';
    reservationSettlementSeconds = reservationSettlementSeconds;
    #bucket;
    #brokerPublicUrl;
    #capabilityKey;
    #now;
    ...
    constructor(config, dependencies = {}) {
        ...
        this.#send =
            dependencies.send ??
                ((command, options) => client.send(command, options?.abortSignal ? { abortSignal: options.abortSignal } : undefined));
    }
    async inspect(input) {
    ...
    async issueGrant(input) {
        ...
        if (requestedCapabilityExpiry > input.expiresAt) {
            throw new S3VolumeObjectStoreContractError('Volume capability cannot outlive its durable reservation.');
        }
        ...
        if (input.method === 'PUT') {
            if (!Number.isSafeInteger(input.maximumBytes) ||
                ...
                !input.reservationId) {
                throw new S3VolumeObjectStoreContractError('Volume PUT grant requires a bounded durable reservation.');
            }
            ...
            const revisions = await this.#verifiedRevisions(input.organizationId, input.projectId, prefix);
            const latest = revisions.sort((left, right) => right.lastModified.getTime() - left.lastModified.getTime() ||
                right.key.localeCompare(left.key))[0];
            claims = latest
                ? {
                    method: 'GET',
                    organization_id: input.organizationId,
    ...
    async scheduleDeletion(input) {
        const prefix = volumePrefix(input.organizationId, input.projectId, input.objectPrefix);
        const now = this.#now();
        if (!Number.isFinite(input.deleteAfter.getTime()) ||
            input.deleteAfter.getTime() > now.getTime()) {
            throw new S3VolumeObjectStoreContractError('Volume deletion cannot run before its retention boundary.');
        }
        ...
        for (let pass = 0; pass < maximumDeletePasses; pass += 1) {
            const listed = (await this.#send(new ListObjectVersionsCommand({ Bucket: this.#bucket, Prefix: prefix, MaxKeys: 1_000 })));
            const entries = [...(listed.Versions ?? []), ...(listed.DeleteMarkers ?? [])];
            if (!entries.length)
                break;
            if (entries.some((entry) => !entry.Key?.startsWith(prefix) ||
                !entry.VersionId ||
                entry.VersionId.length > 1_024 ||
                /[\u0000-\u001f\u007f]/.test(entry.VersionId))) {
                throw new S3VolumeObjectStoreContractError('Object storage returned an invalid version during volume deletion.');
            }
            await this.#deleteObjects(entries.map((entry) => ({ Key: entry.Key, VersionId: entry.VersionId })));
            if (pass === maximumDeletePasses - 1) {
                throw new S3VolumeObjectStoreContractError('Volume version deletion did not converge.');
            }
        }
        for (let pass = 0; pass < maximumDeletePasses; pass += 1) {
            const listed = (await this.#send(new ListObjectsV2Command({ Bucket: this.#bucket, Prefix: prefix, MaxKeys: 1_000 })));
            const entries = (listed.Contents ?? []).map(({ Key }) => Key);
            if (!entries.length)
                return;
            if (entries.some((key) => !key?.startsWith(prefix))) {
                throw new S3VolumeObjectStoreContractError('Object storage returned a cross-prefix key during volume deletion.');
            }
    ...
    async handleRequest(request, response) {
        ...
        try {
            ...
            jsonProblem(response, 401, 'invalid_volume_capability', 'The volume capability is invalid or expired.');
            return;
        }
        if (request.method !== capability.method) {
            jsonProblem(response, 405, 'volume_method_denied', 'The capability does not allow this method.');
            return;
        }
        try {
            if (request.method === 'PUT')
                await this.#handlePut(request, response, capability);
            else
                await this.#handleGet(request, response, capability);
        }
        catch (error) {
            ...
            if (isPreconditionFailed(error)) {
                jsonProblem(response, 409, 'volume_revision_conflict', 'This immutable volume revision already exists.');
                return;
            }
            ...
            jsonProblem(response, 502, 'volume_storage_unavailable', 'Durable volume storage is unavailable.', true);
        }
    }
    #grantExpiry(requestedExpiresAt) {
        const now = this.#now();
        ...
        if (ttlSeconds < 1 ||
            ...
            throw new S3VolumeObjectStoreContractError('Volume grant expiry must be between 1 and 900 seconds.');
        }
        return { expiresAt, expiresEpoch };
    }
    async #verifiedRevisions(organizationId, projectId, objectPrefix) {
        const revisions = [];
        let continuationToken;
        const seenTokens = new Set();
        do {
            const page = (await this.#send(new ListObjectsV2Command({
                Bucket: this.#bucket,
            ...
            for (const item of page.Contents ?? []) {
                if (!item.Key ||
                    ...
                    throw new S3VolumeObjectStoreContractError('Object storage returned an invalid volume revision listing.');
                }
                revisions.push({
                    key: item.Key,
                    sizeBytes: item.Size,
            ...
            if (!page.NextContinuationToken || seenTokens.has(page.NextContinuationToken)) {
                throw new S3VolumeObjectStoreContractError('Object storage returned an invalid volume listing cursor.');
            }
        ...
        for (let offset = 0; offset < revisions.length; offset += headConcurrency) {
            const heads = await Promise.all(revisions.slice(offset, offset + headConcurrency).map(async (revision) => ({
                ...revision,
            ...
            for (const revision of heads) {
                if (revision.sizeBytes !== revisions[verified.length].sizeBytes) {
                    throw new S3VolumeObjectStoreContractError('Volume revision changed while its allocation was inspected.');
                }
                observedSizeBytes += revision.sizeBytes;
                if (!Number.isSafeInteger(observedSizeBytes) || observedSizeBytes > maximumVolumeBytes) {
                    throw new S3VolumeObjectStoreContractError('Stored volume revisions exceed the maximum supported allocation.');
                }
    ...
    async #headRevision(organizationId, projectId, objectPrefix, objectKey, abortSignal) {
        if (!validRevisionKey(objectPrefix, objectKey)) {
            throw new S3VolumeObjectStoreContractError('Volume revision key is outside its tenant scope.');
        }
        const reservationId = objectKey.slice(revisionPrefix(objectPrefix).length, -'.bin'.length);
        const head = (await this.#send(new HeadObjectCommand({ Bucket: this.#bucket, Key: objectKey, ChecksumMode: 'ENABLED' }), abortSignal ? { abortSignal } : undefined));
        ...
        if (!Number.isSafeInteger(head.ContentLength) ||
            ...
            throw new S3VolumeObjectStoreContractError('Stored volume revision failed its size, encryption, checksum, or tenant contract.');
        }
        return { sizeBytes: head.ContentLength, checksumHex: nativeChecksumHex };
    }
    async #verifyCapability(grantId, encoded) {
        ...
        if (claims.empty === true) {
            if (claims.object_key !== `${revisionPrefix(prefix)}empty.bin` ||
                claims.size_bytes !== undefined ||
                claims.checksum_sha256 !== undefined) {
        ...
        if (!validRevisionKey(prefix, claims.object_key) ||
    ...
    #transferSignal(request, response, expiresAt) {
    ...
    async #handlePut(request, response, capability) {
        if (exactHeader(request, 'content-type') !== 'application/octet-stream' ||
            ...
            throw new S3VolumeObjectStoreContractError('Volume PUT requires an unencoded application/octet-stream body.');
        }
        const declaredLength = exactHeader(request, 'content-length');
        if (!declaredLength || !/^(?:[1-9][0-9]*)$/.test(declaredLength)) {
            throw new S3VolumeObjectStoreContractError('Volume PUT requires one exact Content-Length header.');
        }
        ...
        if (!Number.isSafeInteger(contentLength) ||
            ...
            jsonProblem(response, 413, 'volume_upload_too_large', 'The upload exceeds this grant byte bound.', true);
            return;
        }
        const checksum = exactHeader(request, checksumHeader);
        const checksumMatch = checksum?.match(checksumPattern);
        if (!checksumMatch) {
            throw new S3VolumeObjectStoreContractError('Volume PUT requires x-nehemiah-content-sha256: sha256:<64 lowercase hex>.');
        }
        ...
        let existing;
        try {
            existing = await this.#headRevision(capability.organizationId, capability.projectId, capability.objectPrefix, capability.objectKey, transferSignal);
        }
        catch (error) {
            if (!isNotFound(error))
                throw error;
        ...
        if (existing) {
            if (existing.sizeBytes !== contentLength ||
                !constantTimeEqual(existing.checksumHex, expectedChecksumHex)) {
                jsonProblem(response, 409, 'volume_revision_conflict', 'This immutable reservation already contains a different revision.', true);
                return;
            }
            const actualChecksumHex = await drainAndHash(request, contentLength, capability.maximumBytes, transferSignal);
        ...
        let result;
        try {
            result = (await this.#send(new PutObjectCommand({
                Bucket: this.#bucket,
            ...
            meter.destroy();
            if (transferSignal.aborted) {
                throw new S3VolumeObjectStoreContractError('Volume upload expired or its client disconnected.');
            }
            throw error;
        }
        ...
        try {
            ...
            await this.#deleteFailedUpload(capability.objectKey, result.VersionId);
            throw new S3VolumeObjectStoreContractError('Object storage did not confirm the upload checksum contract.');
        }
        if (transferSignal.aborted ||
            this.#now() >= capability.expiresAt ||
            !constantTimeEqual(actualChecksumHex, expectedChecksumHex) ||
            ...
            await this.#deleteFailedUpload(capability.objectKey, result.VersionId);
            throw new S3VolumeObjectStoreContractError('Object storage did not confirm the upload checksum and encryption contract.');
        }
    ...
    async #handleGet(request, response, capability) {
        if (capability.empty) {
            jsonProblem(response, 404, 'volume_object_not_found', 'The volume has no revisions.');
            return;
        }
        if (request.headersDistinct.range !== undefined ||
        ...
        const expectedReservationId = capability.objectKey.slice(revisionPrefix(capability.objectPrefix).length, -'.bin'.length);
        if (result.ContentLength !== capability.sizeBytes ||
            ...
            throw new S3VolumeObjectStoreContractError('Stored volume revision failed its download contract.');
        }
    ...
    async #deleteFailedUpload(objectKey, versionId) {
        if (versionId !== undefined &&
            (!versionId || versionId.length > 1_024 || /[\u0000-\u001f\u007f]/.test(versionId))) {
            throw new S3VolumeObjectStoreContractError('Object storage returned an invalid upload version identifier.');
        }
    ...
    async #deleteObjects(objects) {
        ...
        if (result.Errors?.length) {
            throw new S3VolumeObjectStoreContractError('Object storage rejected part of the volume deletion batch.');
        }
    }
}
//# sourceMappingURL=s3-volumes.js.map