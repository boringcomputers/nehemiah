...
import { validTemplateChecksum } from '../domain/templates.js';
...
const normalizedOrigins = (origins) => {
    ...
    if (result.size === 0)
        throw new Error('at least one template object origin is required');
    return result;
};
const validateGrant = (grant, method, allowedOrigins, artifact) => {
    let url;
    try {
...
export class NehemiahdTemplateClient {
    credentials;
    fetcher;
    requestTimeoutMs;
    hostPort;
    #origins;
    constructor(credentials, allowedObjectOrigins, fetcher = fetch, requestTimeoutMs = 15 * 60 * 1_000, hostPort = 8080) {
    ...
    async export(input) {
        ...
        const result = await this.#request(input.address, `/internal/v1/machines/${encodeURIComponent(input.hostMachineId)}/template-exports`, {
            method: 'POST',
    ...
    async upload(input) {
        ...
        const result = await this.#request(input.address, `/internal/v1/machines/${encodeURIComponent(input.hostMachineId)}/template-exports/${encodeURIComponent(input.exportId)}/upload`, {
            method: 'POST',
            headers: { 'x-nehemiah-lease-id': input.leaseId },
            body: JSON.stringify({
    ...
    async discard(input) {
        if (!exportIdPattern.test(input.exportId))
            return;
        try {
            await this.#request(input.address, `/internal/v1/machines/${encodeURIComponent(input.hostMachineId)}/template-exports/${encodeURIComponent(input.exportId)}`, {
                method: 'DELETE',
                headers: { 'x-nehemiah-lease-id': input.leaseId }
            });
        }
    ...
    async activate(input) {
        ...
        const result = await this.#request(input.address, `/internal/v1/templates/${encodeURIComponent(input.hostTemplateName)}/activate`, {
            method: 'POST',
            body: JSON.stringify({
    ...
    async #request(address, path, init) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new HostRequestError(response.status, body, response.status >= 500 || response.status === 408 || response.status === 429, code);
            }
            ...
            if (error instanceof HostRequestError)
                throw error;
            throw new HostRequestError(undefined, error instanceof Error ? error.message : String(error), true);
        }
        finally {
            clearTimeout(timer);
        }
...
//# sourceMappingURL=nehemiahd-templates.js.map