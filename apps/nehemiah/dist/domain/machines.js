...
import { EgressPolicy } from './network-policy.js';
import { validateResources } from '../scheduler/capacity.js';
import { CapacityUnavailable, QuotaExceeded } from '../scheduler/scheduler.js';
const fromExtendOperationRow = (row, replayed) => ({
    id: row.id,
...
export class PostgresMachineRepository {
    ...
    async findByIdempotency(organizationId, projectId, key) {
        const result = await this.database.query(`${selectMachine} WHERE m.organization_id = $1 AND m.project_id = $2
			 AND m.idempotency_key = $3`, [organizationId, projectId, key]);
        return result.rows[0] ? fromRow(result.rows[0]) : undefined;
    }
    ...
    async insertRequested(machine, idempotencyKey) {
        await this.database.transaction(async (client) => {
            if (machine.templateId) {
                ...
                const template = await client.query(`SELECT t.id FROM templates t
					 WHERE t.id = $1 AND t.organization_id = $2 AND t.project_id = $3
					   AND t.deleted_at IS NULL
					 FOR SHARE OF t`, [machine.templateId, machine.organizationId, machine.projectId]);
                if (!template.rowCount)
                    throw new Error('template is not available to this project');
            }
            await client.query(`INSERT INTO machines
            ...
            await this.event(machine.id, machine.organizationId, undefined, 'requested', undefined, client);
        });
    }
    async claimCreate(machineId) {
        const claimToken = randomUUID();
        const result = await this.database.query(`UPDATE machines
			 ...
			 RETURNING create_claim_token`, [machineId, claimToken]);
        return result.rows[0]?.create_claim_token;
    }
    async releaseCreateClaim(machineId, claimToken) {
        await this.database.query(`UPDATE machines SET create_claim_token = NULL, create_claimed_until = NULL
			 WHERE id = $1 AND create_claim_token = $2`, [machineId, claimToken]);
    }
    async failCreate(machineId, claimToken, status, code, message) {
        return this.database.transaction(async (client) => {
            const result = await client.query(`WITH prior AS (
				...
				) SELECT * FROM updated`, [machineId, claimToken, status, code, message]);
    ...
    async assign(machineId, reservation) {
        await this.database.transaction(async (client) => {
            const result = await client.query(`UPDATE machines SET host_id = $2, runtime_cohort_id = $3,
    ...
    async observeHost(machineId, hostMachine) {
        ...
        await this.database.transaction(async (client) => {
            const result = await client.query(`UPDATE machines SET host_machine_id = $2, state = $3, ready = $4,
            ...
            if (ready && result.rows[0]) {
                await this.event(machineId, result.rows[0].organization_id, 'starting', 'running', undefined, client);
            }
        });
    }
    async transition(machineId, from, to, reason) {
        return this.database.transaction(async (client) => {
            const result = await client.query(`WITH prior AS (SELECT id, organization_id, state AS old_state FROM machines
    ...
    async beginExtend(machineId, organizationId, projectId, idempotencyKey, requestHash, ttlSeconds) {
        return this.database.transaction(async (client) => {
            const organization = await client.query('SELECT id FROM organizations WHERE id = $1 FOR UPDATE', [organizationId]);
            if (!organization.rows[0])
                return undefined;
            const locked = await client.query(`SELECT expires_at, state, host_id, host_machine_id, vcpus, memory_mb
				 FROM machines
				 WHERE id = $1 AND organization_id = $2 AND project_id = $3
				 FOR UPDATE`, [machineId, organizationId, projectId]);
            if (!locked.rows[0])
                return undefined;
            const prior = await client.query(`SELECT id::text, request_hash, target_expires_at, result_expires_at
				 FROM machine_extend_operations
				 WHERE organization_id = $1 AND machine_id = $2 AND idempotency_key = $3`, [organizationId, machineId, idempotencyKey]);
            if (prior.rows[0])
                return fromExtendOperationRow(prior.rows[0], true);
            if (!['starting', 'running'].includes(locked.rows[0].state) ||
                !locked.rows[0].host_id ||
                !locked.rows[0].host_machine_id) {
                return undefined;
            }
            const target = await client.query(`WITH admission_clock AS MATERIALIZED (
				 ...
				 FROM target`, [locked.rows[0].expires_at, organizationId, machineId, ttlSeconds]);
            const targetExpiresAt = target.rows[0]?.target_expires_at;
            if (!targetExpiresAt)
                throw new Error('could not allocate an extend target');
            ...
            const inserted = await client.query(`INSERT INTO machine_extend_operations
				 (organization_id, project_id, machine_id, idempotency_key, request_hash,
				  target_expires_at)
				 VALUES ($1, $2, $3, $4, $5, $6)
				 RETURNING id::text, request_hash, target_expires_at, result_expires_at`, [organizationId, projectId, machineId, idempotencyKey, requestHash, targetExpiresAt]);
            return fromExtendOperationRow(inserted.rows[0], false);
        });
    }
    async completeExtend(operationId, machineId, organizationId, resultExpiresAt) {
        return this.database.transaction(async (client) => {
            const locked = await client.query(`SELECT id FROM machines WHERE id = $1 AND organization_id = $2 FOR UPDATE`, [machineId, organizationId]);
            if (!locked.rows[0])
                throw new Error('machine disappeared while completing extend');
            const completed = await client.query(`UPDATE machine_extend_operations
				 SET result_expires_at = $4, completed_at = now()
				 WHERE id = $1::bigint AND machine_id = $2 AND organization_id = $3
				   AND completed_at IS NULL
				 RETURNING result_expires_at`, [operationId, machineId, organizationId, resultExpiresAt]);
            const recorded = completed.rows[0] ??
                (await client.query(`SELECT result_expires_at FROM machine_extend_operations
						 WHERE id = $1::bigint AND machine_id = $2 AND organization_id = $3
						   AND completed_at IS NOT NULL`, [operationId, machineId, organizationId])).rows[0];
            if (!recorded)
                throw new Error('extend operation disappeared while completing');
            await client.query(`UPDATE machines SET expires_at = GREATEST(expires_at, $3)
				 WHERE id = $1 AND organization_id = $2
				   AND state NOT IN ('stopped', 'failed', 'lost')`, [machineId, organizationId, recorded.result_expires_at]);
            return recorded.result_expires_at;
        });
    }
    async beginFork(sourceMachineId, organizationId, projectId, idempotencyKey, requestHash, count, auditOperationId) {
        return this.database.transaction(async (client) => {
            ...
            const organizationResult = await client.query(`SELECT organization.max_machines, organization.max_vcpus,
				        organization.max_memory_mb, organization.max_disk_mb
				 FROM organizations organization
				 WHERE organization.id = $1 FOR UPDATE OF organization`, [organizationId]);
            const organization = organizationResult.rows[0];
            if (!organization)
                return undefined;
            const limits = await client.query(`SELECT max_machines, max_vcpus, max_memory_mb, max_disk_mb
				 FROM projects WHERE id = $1 AND organization_id = $2 FOR UPDATE`, [projectId, organizationId]);
            if (!limits.rows[0])
                return undefined;
            const sourceResult = await client.query(`SELECT m.*, host(h.address) AS host_address, t.host_template_name,
				 ...
				 FOR UPDATE OF m, h`, [sourceMachineId, organizationId, projectId]);
            const source = sourceResult.rows[0];
            if (!source)
                return undefined;
            const prior = await client.query(`SELECT id FROM machine_fork_operations
				 WHERE organization_id = $1 AND project_id = $2 AND source_machine_id = $3
				   AND idempotency_key = $4`, [organizationId, projectId, sourceMachineId, idempotencyKey]);
            if (prior.rows[0])
                return this.loadForkOperation(client, prior.rows[0].id, true);
            ...
            const usage = await client.query(`SELECT count(*) AS machines, COALESCE(sum(vcpus), 0) AS vcpus,
				        COALESCE(sum(memory_mb), 0) AS memory_mb,
				        COALESCE(sum(disk_mb), 0) AS disk_mb
				 FROM machines WHERE project_id = $1
				   AND state NOT IN ('requested', 'placing', 'stopped', 'failed', 'lost')`, [projectId]);
            const organizationUsage = await client.query(`SELECT count(*) AS machines, COALESCE(sum(vcpus), 0) AS vcpus,
				        COALESCE(sum(memory_mb), 0) AS memory_mb,
				        COALESCE(sum(disk_mb), 0) AS disk_mb
				 FROM machines WHERE organization_id = $1
				   AND state NOT IN ('requested', 'placing', 'stopped', 'failed', 'lost')`, [organizationId]);
            const project = limits.rows[0];
            const active = usage.rows[0];
            const organizationActive = organizationUsage.rows[0];
            if (Number(organizationActive.machines) + count > organization.max_machines ||
            ...
            if (Number(active.machines) + count > project.max_machines ||
            ...
            const availableVcpus = Math.min(source.total_vcpus - source.reserved_vcpus, source.reported_available_vcpus ?? source.total_vcpus - source.reserved_vcpus);
            const availableMemoryMb = Math.min(source.total_memory_mb - source.reserved_memory_mb, source.reported_available_memory_mb ?? source.total_memory_mb - source.reserved_memory_mb);
            const availableDiskMb = Math.min(Number(source.total_disk_mb) - Number(source.reserved_disk_mb), source.reported_available_disk_mb === null
                ? Number(source.total_disk_mb) - Number(source.reserved_disk_mb)
                : Number(source.reported_available_disk_mb));
            if (availableVcpus < neededVcpus ||
            ...
            await client.query(`INSERT INTO machine_fork_operations
            ...
            for (let ordinal = 0; ordinal < count; ordinal += 1) {
                ...
                await client.query(`INSERT INTO machine_fork_children (operation_id, ordinal, machine_id, lease_id)
					 VALUES ($1, $2, $3, $4)`, [operationId, ordinal, machineId, leaseId]);
            ...
            await client.query(`UPDATE hosts SET reserved_vcpus = reserved_vcpus + $2,
				 reserved_memory_mb = reserved_memory_mb + $3,
				 reserved_disk_mb = reserved_disk_mb + $4, updated_at = now()
				 WHERE id = $1`, [source.host_id, neededVcpus, neededMemoryMb, neededDiskMb]);
            return this.loadForkOperation(client, operationId, false);
        });
    }
    async completeFork(operationId, observed) {
        await this.database.transaction(async (client) => {
            const operation = await client.query(`SELECT state, organization_id, source_machine_id,
            ...
            if (!current)
                throw new Error('fork operation disappeared while completing');
            if (current.state !== 'pending' || current.cleanup_requested)
                return;
            const expected = await client.query(`SELECT child.machine_id, child.lease_id, machine.vcpus, machine.memory_mb,
				 ...
				 WHERE child.operation_id = $1 ORDER BY child.ordinal`, [operationId]);
            const byPublicId = new Map(observed.map((machine) => [machine.metadata?.public_machine_id, machine]));
            const hostIds = new Set(observed.map(({ id }) => id));
            if (byPublicId.size !== expected.rows.length ||
                observed.length !== expected.rows.length ||
                hostIds.size !== expected.rows.length ||
                observed.some(({ id }) => id.length === 0)) {
                throw new HostRequestError(undefined, 'host fork response did not contain the full batch', true);
            }
            for (const child of expected.rows) {
                ...
                if (!hostMachine ||
                    ...
                    throw new HostRequestError(undefined, 'host fork child identity or descriptor did not match', true);
                }
            }
            const allReady = observed.every((machine) => machine.ready === true && machine.status === 'running');
            for (const child of expected.rows) {
                const hostMachine = byPublicId.get(child.machine_id);
                const updated = await client.query(`UPDATE machines SET host_machine_id = $2,
                ...
                if (!updated.rowCount)
                    throw new Error('fork child could not be completed');
                if (allReady) {
                    await client.query(`INSERT INTO machine_events
						 (event_key, machine_id, organization_id, from_state, to_state, observed_by)
						 VALUES ($1, $2, $3, 'starting', 'running', 'control-plane')
						 ON CONFLICT DO NOTHING`, [`${child.machine_id}:fork-ready`, child.machine_id, current.organization_id]);
                }
            }
            if (!allReady)
                return;
            await client.query(`UPDATE machine_fork_operations SET state = 'succeeded', completed_at = now(),
				 reconcile_claim_token = NULL, reconcile_claimed_until = NULL
				 WHERE id = $1 AND state = 'pending'`, [operationId]);
    ...
    async failFork(operationId, status, code, message) {
        await this.database.transaction(async (client) => {
            const operation = await client.query(`SELECT state, organization_id, source_host_id,
				        cleanup_requested_at IS NOT NULL AS cleanup_requested
				 FROM machine_fork_operations
				 WHERE id = $1 FOR UPDATE`, [operationId]);
            const current = operation.rows[0];
            if (!current)
                throw new Error('fork operation disappeared while failing');
            if (current.state !== 'pending' || current.cleanup_requested)
                return;
            const resources = await client.query(`SELECT COALESCE(sum(vcpus), 0) AS vcpus,
				        COALESCE(sum(memory_mb), 0) AS memory_mb,
				        COALESCE(sum(disk_mb), 0) AS disk_mb
				 FROM machines WHERE fork_operation_id = $1`, [operationId]);
            await client.query(`WITH prior AS (
				 ...
				 FROM failed ON CONFLICT DO NOTHING`, [operationId, message, current.organization_id]);
            ...
            await client.query(`UPDATE machine_fork_operations SET state = 'failed', failure_status = $2,
				 failure_code = $3, failure_message = $4, completed_at = now(),
				 reconcile_claim_token = NULL, reconcile_claimed_until = NULL,
				 cleanup_claim_token = NULL, cleanup_claimed_until = NULL
				 WHERE id = $1 AND state = 'pending'`, [operationId, status, code, message]);
            await this.recordForkTerminalAudit(client, operationId, 'failed', code);
        });
        return (await this.loadForkOperation(this.database, operationId, true));
    }
    async requestForkCleanup(operationId, status, code, message) {
        let claimedToken;
        await this.database.transaction(async (client) => {
            const operation = await client.query(`SELECT state, cleanup_requested_at, cleanup_claim_token, cleanup_claimed_until
				 FROM machine_fork_operations WHERE id = $1 FOR UPDATE`, [operationId]);
            const current = operation.rows[0];
            if (!current)
                throw new Error('fork operation disappeared while requesting cleanup');
            if (current.state !== 'pending')
                return;
            if (current.cleanup_requested_at === null) {
                await client.query(`UPDATE machine_fork_operations
					 SET cleanup_requested_at = now(), cleanup_failure_status = $2,
					     cleanup_failure_code = $3, cleanup_failure_message = $4,
					     reconcile_after = now()
					 WHERE id = $1 AND state = 'pending' AND cleanup_requested_at IS NULL`, [operationId, status, code, message]);
            }
            const candidateToken = randomUUID();
            const claimed = await client.query(`UPDATE machine_fork_operations
				 ...
				 RETURNING cleanup_claim_token`, [operationId, candidateToken]);
    ...
    async releaseForkCleanupClaim(operationId, cleanupClaimToken) {
        await this.database.query(`UPDATE machine_fork_operations
			 SET cleanup_claim_token = NULL, cleanup_claimed_until = NULL,
			     reconcile_after = now() + interval '15 seconds'
			 WHERE id = $1 AND state = 'pending' AND cleanup_claim_token = $2`, [operationId, cleanupClaimToken]);
    }
    async completeForkCleanup(operationId, cleanupClaimToken) {
        await this.database.transaction(async (client) => {
            const operation = await client.query(`SELECT state, organization_id, source_host_id, cleanup_claim_token,
				        cleanup_failure_status, cleanup_failure_code, cleanup_failure_message
				 FROM machine_fork_operations WHERE id = $1 FOR UPDATE`, [operationId]);
            const current = operation.rows[0];
            if (!current)
                throw new Error('fork operation disappeared while completing cleanup');
            if (current.state !== 'pending')
                return;
            if (current.cleanup_claim_token !== cleanupClaimToken ||
            ...
            await client.query(`WITH prior AS (
				 ...
				 SELECT failed.id || ':fork-failed', failed.id, $3, failed.old_state,
				        'failed', $2, 'control-plane'
				 FROM failed ON CONFLICT DO NOTHING`, [operationId, current.cleanup_failure_message, current.organization_id]);
            const totals = resources.rows[0];
            await client.query(`UPDATE hosts SET reserved_vcpus = GREATEST(0, reserved_vcpus - $2),
				 reserved_memory_mb = GREATEST(0, reserved_memory_mb - $3),
				 reserved_disk_mb = GREATEST(0, reserved_disk_mb - $4), updated_at = now()
            ...
            await this.recordForkTerminalAudit(client, operationId, 'failed', current.cleanup_failure_code);
        });
        return (await this.loadForkOperation(this.database, operationId, true));
    }
    async claimPendingForks(limit = 20) {
        const claimToken = randomUUID();
        const ids = await this.database.transaction(async (client) => {
            const claimed = await client.query(`WITH candidates AS (
				 ...
				 RETURNING operation.id`, [Math.min(Math.max(limit, 1), 100), claimToken]);
            return claimed.rows.map(({ id }) => id);
        });
        const operations = await Promise.all(ids.map((id) => this.loadForkOperation(this.database, id, true)));
        return operations.filter((operation) => operation !== undefined);
    }
    async releaseForkClaim(operationId, claimToken, failed) {
        await this.database.query(`UPDATE machine_fork_operations SET reconcile_claim_token = NULL,
			 reconcile_claimed_until = NULL,
			 reconcile_after = CASE WHEN $3 THEN now() + interval '30 seconds'
			                        ELSE reconcile_after END
			 WHERE id = $1 AND reconcile_claim_token = $2`, [operationId, claimToken, failed]);
    }
    async recordForkTerminalAudit(database, operationId, outcome, reasonCode) {
        await database.query(`INSERT INTO audit_events
			 ...
			 ON CONFLICT (event_key) DO NOTHING`, [operationId, outcome, reasonCode ?? null]);
    }
    async loadForkOperation(database, operationId, replayed) {
        const operationResult = await database.query(`SELECT operation.id, operation.audit_operation_id, operation.idempotency_key,
			 ...
			 WHERE operation.id = $1`, [operationId]);
        const operation = operationResult.rows[0];
        if (!operation)
            return undefined;
        const children = await database.query(`${selectMachine}
    ...
    async event(machineId, organizationId, from, to, reason, database = this.database) {
...
export class ReplayedForkFailure extends Error {
    ...
    code;
    auditOperationId;
    constructor(status, code, message, auditOperationId) {
        super(message);
        this.status = status;
        this.code = code;
...
const failureForCreateError = (error) => {
...
const throwPersistedCreateFailure = (machine) => {
    if (machine.createFailureStatus !== undefined &&
        machine.createFailureCode !== undefined &&
        machine.createFailureMessage !== undefined) {
        throw new ReplayedCreateFailure(machine.createFailureStatus, machine.createFailureCode, machine.createFailureMessage);
    }
};
const throwPersistedForkFailure = (operation) => {
    if (operation.state === 'failed' &&
        operation.failureStatus !== undefined &&
        operation.failureCode !== undefined &&
        operation.failureMessage !== undefined) {
        throw new ReplayedForkFailure(operation.failureStatus, operation.failureCode, operation.failureMessage, operation.auditOperationId);
    }
};
const forkResult = (operation, replayed = operation.replayed) => {
...
export class MachineService {
    ...
    host;
    usage;
    constructor(repository, scheduler, host, usage) {
    ...
    async create(input) {
        ...
        if (imageSources.length !== 1) {
            throw new InvalidMachineRequest('exactly one of template, templateId or ociReference is required');
        }
        if (input.ociReference) {
            throw new InvalidMachineRequest('OCI images are not available until digest-pinned registry imports are enabled');
        }
        if (input.template && !['python', 'desktop'].includes(input.template)) {
            throw new InvalidMachineRequest('template must be a platform built-in; use template_id for published templates');
        }
        ...
        let networkPolicy;
        try {
            networkPolicy = new EgressPolicy(input.networkPolicy ?? { mode: 'off', hostnames: [], cidrs: [] }).declaration;
        }
        catch (error) {
            throw new InvalidMachineRequest(error instanceof Error ? error.message : String(error));
        }
        if (networkPolicy.mode !== 'off') {
            throw new ManagedNetworkEgressUnavailable('Managed guest egress is disabled until hard organization, project, and host-network traffic quotas are enforced.');
        }
        const requestHash = createHash('sha256')
            ...
            .digest('hex');
        const existing = await this.repository.findByIdempotency(input.organizationId, input.projectId, input.idempotencyKey);
        ...
        try {
            ...
            const raced = await this.repository.findByIdempotency(input.organizationId, input.projectId, input.idempotencyKey);
            if (raced) {
                if (raced.idempotencyRequestHash && raced.idempotencyRequestHash !== requestHash) {
                    throw new IdempotencyConflict('Idempotency-Key was already used with a different request.');
                }
                throwPersistedCreateFailure(raced);
                if (raced.state !== 'requested')
        ...
        if (!claimToken) {
            const pending = (await this.repository.find(machine.id, machine.organizationId, machine.projectId)) ??
                machine;
        ...
        let reservation;
        let createdHostMachine;
        const persistedSource = (await this.repository.find(machine.id, machine.organizationId, machine.projectId)) ??
            machine;
        try {
            ...
            if (!reservation.assignmentCommitted)
            ...
            if (reservation && createdHostMachine) {
                try {
                    ...
                    await this.repository.transition(machine.id, ['starting', 'running'], 'stopping', 'create persistence failed; compensation pending');
                    await this.repository.releaseCreateClaim(machine.id, claimToken);
                    throw error;
                }
            }
            const failure = failureForCreateError(error);
            const failed = await this.repository.failCreate(machine.id, claimToken, failure.status, failure.code, failure.message);
    ...
    async destroy(id, organizationId, projectId) {
        ...
        if (machine.hostId && machine.hostAddress && !machine.hostMachineId)
            return true;
        ...
        if (machine.hostId)
            await this.scheduler.release(machine.hostId, machine.resources, machine.id);
        return true;
    }
    async extend(id, organizationId, projectId, ttlSeconds, idempotencyKey) {
        ...
        const machine = await this.repository.find(id, organizationId, projectId);
        if (!machine)
            return { machine: undefined, replayed: false, applied: false };
        if (new EgressPolicy(machine.networkPolicy ?? { mode: 'off' }).declaration.mode !== 'off') {
            throw new ManagedNetworkEgressUnavailable('Machines with a legacy managed egress policy cannot be extended.');
        }
        const requestHash = createHash('sha256')
            .update(JSON.stringify({ machineId: machine.id, ttlSeconds }))
            .digest('hex');
        const operation = await this.repository.beginExtend(id, organizationId, machine.projectId, idempotencyKey, requestHash, ttlSeconds);
        if (!operation)
            return { machine: undefined, replayed: false, applied: false };
        ...
        const hostMachine = await this.host.extend(current.hostAddress, current.hostMachineId, current.leaseId, idempotencyKey, operation.targetExpiresAt);
        ...
        if (!hostExpiry ||
            ...
            throw new HostRequestError(undefined, 'host extend response did not honor the absolute expiry target', true);
        }
        const recordedExpiry = await this.repository.completeExtend(operation.id, id, organizationId, hostExpiry);
    ...
    async fork(id, organizationId, projectId, count, idempotencyKey, auditOperationId = randomUUID()) {
        if (!/^[A-Za-z0-9._:-]{1,128}$/.test(idempotencyKey)) {
            throw new InvalidMachineRequest('A valid Idempotency-Key header is required.');
        }
        ...
        if (!source)
            throw new MachineNotForkable('The source machine was not found.');
        if (new EgressPolicy(source.networkPolicy ?? { mode: 'off' }).declaration.mode !== 'off') {
            throw new ManagedNetworkEgressUnavailable('Machines with a legacy managed egress policy cannot be forked.');
        }
        const requestHash = createHash('sha256')
            .update(JSON.stringify({ sourceMachineId: source.id, count }))
            .digest('hex');
        const operation = await this.repository.beginFork(source.id, organizationId, source.projectId, idempotencyKey, requestHash, count, auditOperationId);
        if (!operation)
            throw new MachineNotForkable('The source machine is not forkable.');
        if (operation.requestHash !== requestHash) {
            throw new IdempotencyConflict('Idempotency-Key was already used with a different request.');
        }
        if (operation.state !== 'pending')
            return forkResult(operation, true);
        const replayed = operation.replayed;
        try {
            let active = operation;
            if (active.cleanupRequested) {
                active = await this.repository.requestForkCleanup(active.id, active.cleanupFailureStatus ?? 502, active.cleanupFailureCode ?? 'fork_batch_failed', active.cleanupFailureMessage ?? 'The fork batch is awaiting confirmed host cleanup.');
                if (active.state !== 'pending')
                    return forkResult(active, replayed);
            }
            let observed;
            try {
                observed = await this.host.fork(active.sourceHostAddress, active.sourceHostMachineId, active.sourceLeaseId, idempotencyKey, active.children.map((child) => ({
                ...
                if (error instanceof HostRequestError && error.code === 'fork_batch_cleaned') {
                    return this.finishForkCleanup(active, {
                        status: 502,
                        code: 'fork_batch_failed',
                        message: 'The host returned an incomplete fork batch and removed all reserved children.'
                    }, [], replayed, true);
                }
                ...
                if (error instanceof HostRequestError && error.ambiguous) {
                    ...
                    return this.finishForkCleanup({ ...active, cleanupClaimToken: undefined }, {
                        status: 502,
                        code: 'fork_batch_failed',
                        message: 'The host result was ambiguous; the fork remains hidden until cleanup is confirmed.'
                    }, [], replayed);
                }
                if (active.cleanupRequested) {
                    if (active.cleanupClaimToken) {
                ...
                const sourceRejected = error instanceof HostRequestError && [404, 409, 422, 501].includes(error.status ?? 0);
                const failure = error instanceof HostRequestError
                    ? {
                ...
                const failed = await this.repository.failFork(active.id, failure.status, failure.code, failure.message);
            ...
            if (active.cleanupRequested) {
                return this.finishForkCleanup(active, {
                    ...
                    message: active.cleanupFailureMessage ?? 'The fork batch is awaiting confirmed host cleanup.'
                }, observed, replayed);
            }
            ...
            if (terminal || (!allReady && operation.deadlineReached)) {
                ...
                return this.finishForkCleanup(active, { status: 502, code: 'fork_batch_failed', message }, observed, replayed);
            }
            let completed;
            try {
    ...
    async finishForkCleanup(operation, failure, observed, replayed, hostConfirmed = false) {
        const cleanup = operation.cleanupRequested && operation.cleanupClaimToken
            ? operation
            : await this.repository.requestForkCleanup(operation.id, failure.status, failure.code, failure.message);
        if (cleanup.state !== 'pending')
            return forkResult(cleanup, replayed);
        if (!cleanup.cleanupClaimToken)
            return forkResult(cleanup, replayed);
        ...
        const terminal = await this.repository.completeForkCleanup(cleanup.id, cleanup.cleanupClaimToken);
    ...
    async exec(id, organizationId, projectId, command, timeoutSeconds) {
        ...
        if (!machine?.ready || !machine.hostAddress || !machine.hostMachineId)
            return undefined;
        return this.host.exec(machine.hostAddress, machine.hostMachineId, machine.leaseId, command, timeoutSeconds);
    }
}
