...
const defaultRequestRateConfig = {
    windowSeconds: 60,
...
const requestSlot = (scope, identifier) => createHash('sha256').update(scope).update('\0').update(identifier).digest().readUInt32BE(0) &
...
export class InvalidStreamAdmission extends Error {
    code = 'invalid_stream_admission';
}
const validate = (request) => {
    if (!uuid.test(request.id) ||
    ...
    if (!identity.test(request.instanceId) ||
...
export class StreamAdmissionService {
    database;
    requestRate;
    static leaseSeconds = 15;
    constructor(database, requestRate = defaultRequestRateConfig) {
        this.database = database;
        this.requestRate = requestRate;
        if (!Number.isSafeInteger(requestRate.windowSeconds) ||
     ...
     */
    async admitRouteRequest(request) {
        validate(request);
        const scopes = [
            {
                scope: 'authority',
        ...
        const result = await this.database.query(`WITH exact_active_lease AS (
        ...
        if (!row?.allowed) {
            ...
            throw new StreamRequestRateExceeded(Number.isSafeInteger(retryAfter) && retryAfter > 0
    ...
    async acquire(request) {
        ...
        return this.database.transaction(async (client) => {
            const organization = await client.query(`SELECT max_gateway_streams, max_gateway_bandwidth_bps::text
				 FROM organizations WHERE id = $1 FOR UPDATE`, [request.organizationId]);
            const project = await client.query(`SELECT max_gateway_streams, max_gateway_bandwidth_bps::text
				 FROM projects WHERE id = $1 AND organization_id = $2 FOR UPDATE`, [request.projectId, request.organizationId]);
            ...
            await client.query(`DELETE FROM gateway_stream_leases
				 WHERE organization_id = $1 AND expires_at <= statement_timestamp()`, [request.organizationId]);
            const existing = await client.query(`SELECT organization_id::text, project_id::text, authority_kind, authority_id,
				        gateway_instance_id, bandwidth_bytes_per_second::text
				 FROM gateway_stream_leases WHERE id = $1 FOR UPDATE`, [request.id]);
            const current = existing.rows[0];
            if (current) {
                if (current.organization_id !== request.organizationId ||
            ...
            const usage = await client.query(`SELECT
				 ...
				 WHERE organization_id = $1 AND expires_at > statement_timestamp()`, [request.organizationId, request.projectId]);
            ...
            if (consumed.organization_streams + additionalStreams > organizationLimit.max_gateway_streams ||
            ...
            const result = await client.query(`INSERT INTO gateway_stream_leases
    ...
    async release(id, instanceId) {
        if (!uuid.test(id) || !identity.test(instanceId)) {
            throw new InvalidStreamAdmission('stream release identity is invalid');
        }
        await this.database.query('DELETE FROM gateway_stream_leases WHERE id = $1 AND gateway_instance_id = $2', [id, instanceId]);
    }
    async reapExpired(limit = 1_000) {
        if (!Number.isSafeInteger(limit) || limit < 1 || limit > 10_000) {
            throw new InvalidStreamAdmission('stream reap limit is invalid');
        }
        const result = await this.database.query(`DELETE FROM gateway_stream_leases WHERE id IN (
...
//# sourceMappingURL=stream-admission.js.map