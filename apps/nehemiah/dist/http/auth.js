...
import { apiKeyPublicPrefix, ApiKeyVerifierBusy } from '../auth/api-key.js';
import { ApiRateLimitExceeded, coarseClientIdentity } from '../auth/api-admission.js';
...
const tokenFingerprint = (token) => createHash('sha256').update('nehemiah-auth-attempt-v1\0').update(token).digest('hex');
const classifyPresentedCredential = (token) => {
...
const recordAttempt = async (request, services, attempt, input) => {
...
const authenticateRawIdentity = async (request, services) => {
...
const admitIdentity = async (identity, services, organizationId = identity.organizationId) => {
    await services.apiAdmission?.authenticate({
        principalId: identity.kind === 'api_key' ? `api_key:${identity.apiKeyId}` : `user:${identity.clerkUserId}`,
        organizationId,
        projectId: identity.kind === 'api_key' ? identity.projectId : undefined
    });
};
const admitIdentityWithAudit = async (request, attempt, identity, services, organizationId = identity.organizationId) => {
    try {
...
export const authenticateIdentity = async (request, services) => {
...
export const auditHttpAuthorizationDenial = async (request, route, status) => {
...
export const permits = (principal, scope) => {
    if (principal.kind === 'api_key')
        return principal.scopes.has(scope);
    if (principal.role === 'owner' || principal.role === 'admin')
        return true;
    if (principal.role === 'billing')
...
export const canAdminister = (principal) => principal.kind === 'user' && (principal.role === 'owner' || principal.role === 'admin');
