import { apiKeyScopes, ApiKeyAllocationUnavailable, ApiKeyAuthorizationDenied, ApiKeyHasherBusy, ApiKeyQuotaExceeded, InvalidApiKeyRequest } from '../../auth/api-key.js';
import { authenticate, canAdminister } from '../auth.js';
import { json, problem, readJson } from '../router.js';
const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const auditContext = (request, requestId) => ({
    requestId,
    userAgent: request.headers.get('user-agent') ?? undefined
});
const hashingBusy = (error, requestId) => {
...
export const registerApiKeyRoutes = (router) => {
    router.get('/v1/api-keys', async ({ request, url, services, requestId }) => {
        ...
        if (!principal || principal.kind !== 'user' || !canAdminister(principal)) {
            return problem(403, 'administrator_required', 'An owner or admin role is required.', requestId);
        }
        const keys = await services.apiKeys.list(principal.organizationId, url.searchParams.get('project_id') ?? undefined);
        return json({
    ...
    router.post('/v1/api-keys', async ({ request, services, requestId }) => {
        ...
        try {
            ...
            if (error instanceof ApiKeyAllocationUnavailable) {
                return problem(503, error.code, 'Could not allocate a unique API-key prefix. Retry later.', requestId);
            }
            throw error;
        }
    });
    const setDisabled = (disabled) => async ({ request, params, services, requestId }) => {
        const principal = await authenticate(request, services);
        if (!principal || principal.kind !== 'user' || !canAdminister(principal)) {
            return problem(403, 'administrator_required', 'An owner or admin role is required.', requestId);
        }
        ...
        try {
            const accepted = disabled
                ? await services.apiKeys.disable(params.id, principal.organizationId, principal.clerkUserId, auditContext(request, requestId))
                : await services.apiKeys.enable(params.id, principal.organizationId, principal.clerkUserId, auditContext(request, requestId));
            return accepted
    ...
    router.post('/v1/api-keys/:id/rotate', async ({ request, params, services, requestId }) => {
        ...
        try {
            const replacement = await services.apiKeys.rotate(params.id, principal.organizationId, principal.clerkUserId, auditContext(request, requestId));
            return replacement
                ? json({
                    ...replacement,
    ...
    router.delete('/v1/api-keys/:id', async ({ request, params, services, requestId }) => {
        ...
        try {
            const revoked = await services.apiKeys.revoke(params.id, principal.organizationId, principal.clerkUserId, auditContext(request, requestId));
            return revoked
                ? new Response(null, { status: 204 })
