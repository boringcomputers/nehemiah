...
import { InvalidVolumeRequest, volumeGrantJson, volumeJson, VolumeIdempotencyConflict, VolumeInfrastructureUnavailable, VolumeIntegrityError, VolumeProjectUnavailable, VolumeQuotaExceeded } from '../../domain/volumes.js';
...
const crossProject = (principal, requested) => principal.kind === 'api_key' &&
...
const volumeProblem = (error, requestId) => {
...
export const registerVolumeRoutes = (router) => {
    router.post('/v1/volumes', async ({ request, services, requestId }) => {
        ...
        try {
            ...
            if (typeof body !== 'object' ||
                body === null ||
                Array.isArray(body) ||
                Object.keys(body).some((key) => !['project_id', 'size_limit_mb', 'ttl_seconds', 'grant_ttl_seconds'].includes(key))) {
                throw new InvalidVolumeRequest('Only project_id, size_limit_mb, and TTL fields are accepted.');
            }
            ...
            return json({ ...volumeJson(created.volume), grant: volumeGrantJson(created.grant) }, created.replayed ? 200 : 201, {
            ...
            return (volumeProblem(error, requestId) ??
    ...
    router.get('/v1/volumes', async ({ request, url, services, requestId }) => {
        ...
        return json({
            volumes: (await services.volumes.list(principal.organizationId, projectId)).map((volume) => volumeJson(volume))
        });
    });
    router.get('/v1/volumes/:id', async ({ request, params, services, requestId }) => {
        ...
        const volume = await services.volumes.get(params.id, principal.organizationId, projectFor(principal));
        return volume
    ...
    router.post('/v1/volumes/:id/grants', async ({ request, params, services, requestId }) => {
        ...
        try {
            ...
            }, () => services.volumes.grant(params.id, principal.organizationId, projectFor(principal), body.method, body.ttl_seconds));
            return result
