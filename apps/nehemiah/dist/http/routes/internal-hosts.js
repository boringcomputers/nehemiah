...
import { canonicalGatewayCapabilities, gatewayCapabilities } from '../../auth/capability.js';
import { MeteringHostLifecycleError, MeteringInputError, MeteringRateLimitError } from '../../billing/metering.js';
import { HostEnrollmentError, HostHeartbeatError } from '../../domain/hosts.js';
import { HostCredentialAdmission, HostCredentialRateLimitExceeded, HostCredentialVerifierBusy } from '../../auth/host-admission.js';
import { InvalidRuntimeCohort, parseRuntimeCohort } from '../../domain/runtime-cohort.js';
import { InvalidStreamAdmission, StreamAdmissionRejected, StreamRequestRateExceeded } from '../../domain/stream-admission.js';
...
const authenticateHost = async (request, hostId, credential, hosts, requestId) => {
    if (!credential) {
        return problem(401, 'unauthorized', 'The host credential is invalid.', requestId);
    }
    try {
        const generation = await hostCredentialAdmission.verify(request, hostId, () => hosts.authenticateGeneration(hostId, credential));
...
const operator = async (request, services, requestId, action, hostId) => {
...
const reasonBody = async (request, requestId) => {
    try {
        ...
        if (typeof body !== 'object' ||
            ...
            return problem(400, 'invalid_host_lifecycle_request', 'reason must be a non-empty string of at most 512 characters.', requestId);
        }
        return { reason: typeof body.reason === 'string' ? body.reason : undefined };
    }
    catch (error) {
        return problem(400, 'invalid_host_lifecycle_request', error instanceof Error ? error.message : 'The request body is invalid.', requestId);
    }
};
export const registerInternalHostRoutes = (router) => {
    router.post('/internal/v1/hosts/register', async ({ request, services, requestId }) => {
        ...
        if (typeof input !== 'object' ||
            ...
            !['provider_id', 'region_id', 'address', 'control_token', 'gateway_token'].every((key) => typeof Reflect.get(input, key) === 'string') ||
            !['x86_64', 'aarch64'].includes(input.architecture) ||
            ![input.total_vcpus, input.total_memory_mb, input.total_disk_mb].every((value) => Number.isSafeInteger(value) && value > 0) ||
            input.control_token.length < 32 ||
            input.gateway_token.length < 32) {
            return problem(400, 'invalid_host_registration', 'The host identity, credentials, and positive resource totals are required.', requestId);
        }
        let runtimeCohort;
        try {
            ...
            if (!(error instanceof InvalidRuntimeCohort))
                throw error;
        ...
        try {
            ...
            if (error instanceof HostEnrollmentError) {
                return problem(error.code === 'invalid_host_enrollment' ? 400 : 401, error.code, error.code === 'invalid_host_enrollment'
                    ? error.message
                    : 'The host enrollment grant is invalid or no longer active.', requestId);
            }
            throw error;
        }
        return json({
    ...
    router.post('/internal/v1/hosts/:id/heartbeat', async ({ request, params, services, requestId }) => {
        const credentialGeneration = await authenticateHost(request, params.id, bearer(request), services.hosts, requestId);
        if (credentialGeneration instanceof Response)
        ...
        if (typeof body !== 'object' ||
            ...
            return problem(400, 'invalid_host_heartbeat', 'The heartbeat must contain the exact bounded host state, capacity, daemon version, and runtime cohort fields.', requestId);
        }
        let runtimeCohort;
        try {
        ...
        let accepted;
        try {
            accepted = await services.hosts.heartbeat(params.id, {
                state: body.state,
            ...
            if (error instanceof HostHeartbeatError) {
                const response = problem(error.code === 'host_heartbeat_rate_limited' ? 429 : 400, error.code, error.message, requestId, error.retryAfterSeconds === undefined
                    ? {}
                    : { retry_after_seconds: error.retryAfterSeconds });
        ...
        if (!accepted) {
            return problem(409, 'host_lifecycle_conflict', 'The host is stale or its desired lifecycle state changed.', requestId);
        }
        return new Response(null, { status: 204 });
    });
    router.post('/internal/v1/hosts/:id/usage-observations', async ({ request, params, services, requestId }) => {
        const credentialGeneration = await authenticateHost(request, params.id, bearer(request), services.hosts, requestId);
        if (credentialGeneration instanceof Response)
            return credentialGeneration;
        const metering = services.metering;
        if (!metering) {
            return problem(503, 'metering_unavailable', 'The authoritative metering ingestor is unavailable.', requestId);
        }
        let raw;
        try {
            ...
            return problem(error instanceof Error && 'status' in error && typeof error.status === 'number'
                ? error.status
                : 400, 'invalid_metering_batch', 'The metering observation batch is invalid.', requestId);
        }
        if (typeof raw !== 'object' ||
            ...
            return problem(400, 'invalid_metering_batch', 'The body must contain only an observations array.', requestId);
        }
        const fields = new Set([
            'machine_id',
            'host_machine_id',
        ...
        const observations = [];
        for (const item of Reflect.get(raw, 'observations')) {
            if (typeof item !== 'object' ||
                item === null ||
                Array.isArray(item) ||
                Object.keys(item).length !== fields.size ||
                Object.keys(item).some((key) => !fields.has(key))) {
                return problem(400, 'invalid_metering_batch', 'Every observation must contain exactly the metering contract fields.', requestId);
            }
            const value = item;
            if (![
                'machine_id',
                ...
                return problem(400, 'invalid_metering_batch', 'Meter counters and identifiers must use their documented JSON types.', requestId);
            }
            observations.push({
                machineId: value.machine_id,
                hostMachineId: value.host_machine_id,
                leaseId: value.lease_id,
                leaseGeneration: value.lease_generation,
                hostBootId: value.host_boot_id,
                sequence: value.sequence,
                kind: value.kind,
                quality: value.quality,
                qualityReason: value.quality_reason,
                runtimeNs: value.runtime_ns,
                egressBytes: value.egress_bytes,
                egressCounterEpoch: value.egress_counter_epoch,
                observedMonotonicNs: value.observed_monotonic_ns,
        ...
        try {
            ...
            if (error instanceof MeteringHostLifecycleError) {
                return problem(409, 'host_lifecycle_conflict', 'The host lifecycle no longer accepts observations.', requestId);
            }
            throw error;
        }
    });
    router.post('/v1/operator/host-enrollments', async ({ request, services, requestId }) => {
        const principal = await operator(request, services, requestId, 'host.enrollment.issue', 'new-host');
        ...
        if (typeof body !== 'object' ||
            ...
            Array.isArray(body) ||
            Object.keys(body).some((key) => !fields.has(key)) ||
            !['provider_id', 'region_id', 'address'].every((key) => typeof body[key] === 'string' && String(body[key]).length > 0) ||
            !['x86_64', 'aarch64'].includes(String(body.architecture)) ||
            ![body.total_vcpus, body.total_memory_mb, body.total_disk_mb].every((value) => Number.isSafeInteger(value) && Number(value) > 0) ||
            (body.ttl_seconds !== undefined &&
                (!Number.isSafeInteger(body.ttl_seconds) ||
                    Number(body.ttl_seconds) < 60 ||
                    Number(body.ttl_seconds) > 1800))) {
            return problem(400, 'invalid_host_enrollment', 'A provider, region, overlay address, architecture, positive resource totals, and optional 60-1800 second TTL are required.', requestId);
        }
        let runtimeCohort;
        try {
            runtimeCohort = parseRuntimeCohort(body.runtime_cohort, body.architecture);
        }
        catch (error) {
            if (!(error instanceof InvalidRuntimeCohort))
                throw error;
        ...
        try {
            ...
            if (error instanceof HostEnrollmentError) {
                return problem(error.code === 'host_not_enrollable' ? 409 : 400, error.code, error.message, requestId);
            }
            throw error;
        }
    });
    router.post('/v1/operator/host-enrollments/:id/revoke', async ({ request, params, services, requestId }) => {
        if (!uuid.test(params.id)) {
            return problem(400, 'invalid_host_enrollment', 'A valid enrollment ID is required.', requestId);
        }
        const principal = await operator(request, services, requestId, 'host.enrollment.revoke', params.id);
        if (principal instanceof Response)
            return principal;
        const revoked = await services.audit.capture({
            ...
            complete: (accepted) => accepted ? {} : { outcome: 'denied', reasonCode: 'host_enrollment_not_active' }
        }, () => services.hosts.revokeEnrollment(params.id));
        return revoked
            ? new Response(null, { status: 204 })
            : problem(409, 'host_enrollment_not_active', 'The enrollment grant is unknown or no longer active.', requestId);
    });
    ...
    for (const operation of [
        {
            path: 'drain',
            action: 'host.drain',
            run: (hosts, id, reason) => hosts.drain(id, reason)
        },
        {
            path: 'activate',
            action: 'host.activate',
            run: (hosts, id, reason) => hosts.activate(id, reason)
        },
        {
            path: 'quarantine',
            action: 'host.quarantine',
            run: (hosts, id, reason) => hosts.quarantine(id, reason)
        },
        {
            path: 'revoke',
            action: 'host.revoke',
            run: (hosts, id, reason) => hosts.revoke(id, reason)
        }
    ]) {
        router.post(`/v1/operator/hosts/:id/${operation.path}`, async ({ request, params, services, requestId }) => {
            const principal = await operator(request, services, requestId, operation.action, params.id);
            if (principal instanceof Response)
                return principal;
            ...
            return host
                ...
                : problem(409, 'host_transition_denied', 'The host does not exist or cannot make that lifecycle transition.', requestId);
        });
    }
    router.post('/v1/operator/hosts/:id/credentials/rotate', async ({ request, params, services, requestId }) => {
        const principal = await operator(request, services, requestId, 'host.credentials.rotate', params.id);
        if (principal instanceof Response)
            return principal;
        let body;
        try {
            ...
            return problem(400, 'invalid_host_credential_rotation', error instanceof Error ? error.message : 'The request body is invalid.', requestId);
        }
        if (typeof body !== 'object' ||
            body === null ||
            Array.isArray(body) ||
            Object.keys(body).some((key) => !['control_token', 'gateway_token', 'reason'].includes(key)) ||
            ...
            (body.reason !== undefined &&
                (typeof body.reason !== 'string' ||
                    body.reason.trim().length === 0 ||
                    body.reason.length > 512))) {
            return problem(400, 'invalid_host_credential_rotation', 'control_token and gateway_token must be distinct and contain at least 32 characters; reason is optional and bounded to 512 characters.', requestId);
        }
        ...
        if (!host) {
            return problem(409, 'host_credential_rotation_denied', 'A revoked or unknown host cannot rotate credentials.', requestId);
        }
        return json({
    ...
    router.get('/internal/v1/routing/machines/:id', async ({ request, params, services, requestId }) => {
        ...
        const streamBandwidthText = request.headers.get('x-nehemiah-stream-bandwidth-bytes-per-second');
        ...
        const capabilities = canonicalGatewayCapabilities(requestedCapabilities.filter((value) => gatewayCapabilities.includes(value)));
        ...
        if (!capability ||
            ...
            (capabilities.includes('preview')
                ...
                : portText !== null)) {
            return problem(400, 'invalid_route_capability', 'The complete signed capability scope and exact preview port are required.', requestId);
        }
        if (!services.streamAdmission) {
            return problem(503, 'stream_admission_unavailable', 'Global stream admission is unavailable.', requestId);
        }
        try {
        ...
        const result = await services.database.query(`SELECT h.id AS host_id, host(h.address) AS host_address,
        ...
        if (!hostToken) {
            return problem(502, 'host_credential_unavailable', 'The host route is not usable.', requestId);
        }
        let stream;
        try {
    ...
    router.delete('/internal/v1/routing/streams/:id', async ({ request, params, services, requestId }) => {
        if (!equal(bearer(request), services.gatewayToken)) {
            return problem(401, 'unauthorized', 'The gateway credential is invalid.', requestId);
        }
