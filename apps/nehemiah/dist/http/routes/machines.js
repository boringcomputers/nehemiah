...
import { canonicalGatewayCapabilities, gatewayCapabilities, issueCapabilityToken } from '../../auth/capability.js';
import { BillingAdmissionRejected } from '../../billing/admission.js';
import { HostRequestError } from '../../clients/nehemiahd.js';
import { IdempotencyConflict, InvalidMachineRequest, ManagedNetworkEgressUnavailable, MachineNotForkable, machineJson, ReplayedCreateFailure, ReplayedForkFailure } from '../../domain/machines.js';
...
class MachineSessionLeaseUnavailable extends Error {
    code = 'machine_session_lease_unavailable';
}
...
const previewUrl = (services, machineId, leaseId, port, token) => {
...
export const registerMachineRoutes = (router) => {
    router.post('/v1/machines', async ({ request, services, requestId }) => {
        ...
        try {
            ...
            if (body.oci_reference !== undefined) {
                return problem(501, 'not_supported', 'Managed OCI image imports are not implemented. Use a built-in template or template_id.', requestId);
            }
            ...
            return json(machineJson(result.machine), result.machine.ready ? (result.replayed ? 200 : 201) : 202, result.replayed ? { 'idempotency-replayed': 'true' } : undefined);
        }
        catch (error) {
            ...
            if (error instanceof HostRequestError) {
                return problem(error.ambiguous ? 503 : 502, error.ambiguous ? 'host_result_pending' : 'host_request_failed', error.ambiguous
                    ? 'The host result is not yet known; retry with the same Idempotency-Key.'
                    : 'The selected host rejected the machine request.', requestId);
            }
            return problem(500, 'internal_error', 'The machine request could not be completed.', requestId);
        }
    });
    ...
    router.delete('/v1/machines/:id', async ({ request, params, services, requestId }) => {
        ...
        }, () => services.machines.destroy(result.machine.id, result.principal.organizationId, projectFor(result.principal)));
    ...
    router.post('/v1/machines/:id/extend', async ({ request, params, services, requestId }) => {
        ...
        try {
            ...
            }, () => services.machines.extend(result.machine.id, result.principal.organizationId, projectFor(result.principal), ttlSeconds, idempotencyKey));
            return json(extension.machine ? machineJson(extension.machine) : machineJson(result.machine), 200, extension.replayed ? { 'idempotency-replayed': 'true' } : undefined);
        }
        catch (error) {
            if (error instanceof ManagedNetworkEgressUnavailable)
                return problem(501, error.code, error.message, requestId);
            ...
            if (error instanceof HostRequestError) {
                return problem(error.ambiguous ? 503 : 502, error.ambiguous ? 'host_result_pending' : 'host_request_failed', error.ambiguous
                    ? 'The host result is not yet known; retry with the same Idempotency-Key.'
                    : 'The selected host rejected the extend request.', requestId);
            }
            if (error instanceof BillingAdmissionRejected)
                return problem(429, error.code, error.message, requestId);
            throw error;
    ...
    router.post('/v1/machines/:id/fork', async ({ request, params, services, requestId }) => {
        ...
        try {
            ...
            const forked = await services.audit.capture({
                ...
                failureHandled: (error) => error instanceof ReplayedForkFailure && error.auditOperationId === auditOperationId
            }, () => services.machines.fork(result.machine.id, result.principal.organizationId, projectFor(result.principal), count, idempotencyKey, auditOperationId));
            ...
            return json(payload, forked.pending ? 202 : forked.replayed ? 200 : 201, forked.replayed ? { 'idempotency-replayed': 'true' } : undefined);
        }
        catch (error) {
            if (error instanceof ManagedNetworkEgressUnavailable)
                return problem(501, error.code, error.message, requestId);
            ...
            if (error instanceof CapacityUnavailable)
                return problem(503, error.code, error.message, requestId);
            if (error instanceof HostRequestError) {
                return problem(error.ambiguous ? 503 : error.status === 422 || error.status === 501 ? 422 : 502, error.ambiguous
                    ? 'host_result_pending'
                    : error.status === 422 || error.status === 501
                        ? 'machine_not_forkable'
                        : 'host_request_failed', error.ambiguous
                    ? 'The host result is not yet known; retry with the same Idempotency-Key.'
                    : 'The source host rejected the fork request.', requestId);
            }
    ...
    router.post('/v1/machines/:id/exec', async ({ request, params, services, requestId }) => {
        ...
        if (!Number.isSafeInteger(timeout) || timeout < 1 || timeout > 120) {
            return problem(400, 'invalid_timeout', 'timeout_seconds must be an integer from 1 to 120.', requestId);
        }
        try {
            ...
            }, () => services.machines.exec(result.machine.id, result.principal.organizationId, projectFor(result.principal), body.command, timeout));
            return execution
                ? json(execution)
            ...
            if (error instanceof HostRequestError &&
                ...
                !error.ambiguous) {
                const response = problem(503, error.code, 'The managed guest agent is unavailable; serial recovery is not exposed to tenant requests.', requestId, { retry_after_seconds: 1 });
            ...
            if (error instanceof HostRequestError &&
                ...
                !error.ambiguous) {
                const response = problem(429, error.code, 'The host has reached its bounded guest-operation capacity.', requestId, { retry_after_seconds: 1 });
                response.headers.set('Retry-After', '1');
                return response;
            }
            throw error;
    ...
    router.post('/v1/machines/:id/sessions', async ({ request, params, services, requestId }) => {
        ...
        if (body.capabilities !== undefined && !Array.isArray(body.capabilities)) {
            return problem(400, 'invalid_capability', 'capabilities must be a nonempty array.', requestId);
        }
        const requestedCapabilities = [...new Set(body.capabilities ?? ['tty'])];
        if (requestedCapabilities.length === 0 ||
            requestedCapabilities.some((value) => typeof value !== 'string' || !gatewayCapabilities.includes(value))) {
            return problem(400, 'invalid_capability', 'An unknown capability was requested.', requestId);
        }
        const capabilities = canonicalGatewayCapabilities(requestedCapabilities);
        if (capabilities.includes('agent')) {
            return problem(501, 'not_supported', 'Managed host-local LLM agents are not supported. Run an agent inside the guest through exec, TTY, and file primitives.', requestId);
        }
        const portIsValid = body.port !== undefined &&
        ...
        if (!Number.isSafeInteger(ttl) || ttl < 1 || ttl > 900) {
            return problem(400, 'invalid_ttl', 'ttl_seconds must be an integer from 1 to 900.', requestId);
        }
        const capabilityId = randomUUID();
        const issuerType = result.principal.kind === 'user'
            ? 'clerk_user'
            : result.principal.deviceFamilyId
                ? 'device_family'
                : 'api_key';
        const issuerId = result.principal.kind === 'user'
        ...
        try {
            token = await services.audit.capture({
                ...
                const grant = await services.database.query(`WITH clock AS MATERIALIZED (
                ...
                if (!persisted)
                    throw new MachineSessionLeaseUnavailable();
                return issueCapabilityToken({
                    ...
                    port: body.port
                }, services.gatewaySecret, ttl, { capabilityId, issuedAt: persisted.created_at });
            });
        }
        catch (error) {
            if (error instanceof MachineSessionLeaseUnavailable) {
                return problem(409, 'machine_lease_changed', 'The machine lease changed while the session was being issued.', requestId);
            }
            throw error;
        }
        return json({
    ...
    router.delete('/v1/machines/:id/sessions/:sessionId', async ({ request, params, services, requestId }) => {
        const result = await owned(services, request, params.id, 'machines:write');
        ...
        }, () => services.database.query(`UPDATE machine_gateway_grants
