...
import { DeviceAuthorizationError } from '../../auth/device.js';
import { authenticate, authenticateIdentity } from '../auth.js';
...
const handle = async (requestId, operation) => {
    try {
...
export const registerDeviceAuthorizationRoutes = (router) => {
    router.post('/v1/auth/device/code', ({ request, services, requestId }) => handle(requestId, async () => {
    ...
    router.post('/v1/auth/device/inspect', ({ request, services, requestId }) => handle(requestId, async () => {
        const identity = await authenticateIdentity(request, services);
        if (!identity || identity.kind !== 'user') {
            return problem(401, 'dashboard_session_required', 'Sign in to inspect a device authorization request.', requestId);
        }
        const body = await readJson(request, 16_384);
        if (typeof body.user_code !== 'string') {
            throw new DeviceAuthorizationError('invalid_request', 'A user code is required.');
        }
        return json(await services.deviceAuthorizations.inspect({
            userCode: body.user_code,
            actorId: identity.clerkUserId
        }), 200, noStore);
    }));
    router.post('/v1/auth/device/authorize', ({ request, services, requestId }) => handle(requestId, async () => {
        const principal = await authenticate(request, services);
        if (!principal || principal.kind !== 'user') {
            return problem(401, 'dashboard_session_required', 'Sign in and select an organization to authorize this device.', requestId);
        }
        const body = await readJson(request, 16_384);
        if (typeof body.user_code !== 'string' ||
            (body.decision !== 'approve' && body.decision !== 'deny')) {
            throw new DeviceAuthorizationError('invalid_request', 'A user code and decision are required.');
        }
    ...
    router.post('/v1/auth/device/token', ({ request, services, requestId }) => handle(requestId, async () => {
        const body = await readJson(request, 16_384);
        ...
        return json(await services.deviceAuthorizations.exchange(body.device_code, {
            requestId,
            userAgent: userAgent(request)
        }), 200, noStore);
    }));
    router.post('/v1/auth/device/refresh', ({ request, services, requestId }) => handle(requestId, async () => {
        const body = await readJson(request, 16_384);
        ...
        return json(await services.deviceAuthorizations.refresh(body.refresh_token, {
            requestId,
            userAgent: userAgent(request)
        }), 200, noStore);
    }));
    router.post('/v1/auth/device/revoke', ({ request, services, requestId }) => handle(requestId, async () => {
        const body = await readJson(request, 16_384);
        if (typeof body.refresh_token === 'string' && body.refresh_token.length <= 160) {
...
export const defaultDeviceScopes = apiKeyScopes.filter((scope) => scope !== 'billing:read');
