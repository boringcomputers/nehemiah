import { clientAddressHeader, clientSignatureHeader, clientTimestampHeader, verifiedClientAddress } from './client-identity.js';
...
export class PendingRequestBodyAdmission {
    ...
    #byClient = new Map();
    #total = 0;
    constructor(maximumTotal = maximumPendingRequestBodies, maximumPerClient = maximumPendingBodiesPerClient) {
        if (!Number.isSafeInteger(maximumTotal) ||
    ...
    tryAcquire(client) {
        ...
        return () => {
            ...
            if (remaining === 0)
                ...
                this.#byClient.set(client, remaining);
        };
    }
    snapshot() {
...
export const sanitizedIncomingHeaders = (request, gatewayToken) => {
...
export const admitPendingRequestBody = (request, response, gatewayToken, admission) => {
...
const validateDeclaredBodyLength = (request, maximum) => {
...
export const incomingRequest = async (request, headers, maximumBodyBytes = maximumControlPlaneBodyBytes) => {
    ...
    if ((request.method === 'GET' || request.method === 'HEAD') &&
        ...
        throw new HttpRequestError(400, 'invalid_json', 'GET and HEAD request bodies are not accepted.');
    }
    let body;
    if (request.method !== 'GET' && request.method !== 'HEAD') {
        const chunks = [];
...
export const rejectPendingRequest = (request, response) => {
...
export const closeAfterResponse = (request, response) => {
    response.shouldKeepAlive = false;
    response.setHeader('connection', 'close');
    response.once('finish', () => request.destroy());
};
//# sourceMappingURL=incoming-request.js.map