import { privateBetaRates } from './rates.js';
const microsPerCent = 10000n;
const mebibytesPerGibibyte = 1024n;
const bytesPerGibibyteHour = 1073741824n * 3600n;
...
const decimal = (value) => {
    ...
    if (!match)
        throw new Error('billing quantity is not a non-negative decimal');
...
const ceilingPrice = (quantity, rate, divisor = 1n) => {
    if (rate < 0n || divisor < 1n)
        throw new Error('billing rate configuration is invalid');
    const parsed = typeof quantity === 'bigint' ? { numerator: quantity, denominator: 1n } : decimal(quantity);
    if (parsed.numerator < 0n)
        throw new Error('billing commitment cannot be negative');
...
export class BillingAdmissionPolicy {
    plan;
    #rates;
    constructor(rates = privateBetaRates, plan = 'private_beta') {
        this.plan = plan;
        ...
        for (const dimension of meterDimensions) {
            if (!configured.has(dimension))
                throw new Error(`billing rate is missing for ${dimension}`);
        }
        this.#rates = configured;
    }
    async enforce(client, organizationId, additional = {}) {
        const accountResult = await client.query(`SELECT plan, spend_cap_cents::text, delinquent_at
			 FROM billing_accounts WHERE organization_id = $1 FOR UPDATE`, [organizationId]);
        const account = accountResult.rows[0];
        if (!account)
            return;
        if (account.delinquent_at) {
            throw new BillingAdmissionRejected('billing_delinquent', 'Organization billing is delinquent; new resource commitments are suspended.');
        }
        if (account.plan !== this.plan) {
            throw new BillingAdmissionRejected('billing_plan_unavailable', 'Organization billing plan has no active admission rate card.');
        }
        if (account.spend_cap_cents === null)
            return;
        const usage = await client.query(`WITH durable_usage AS (
			 ...
			 FROM durable_usage GROUP BY dimension`, [organizationId]);
        const commitments = await client.query(`WITH machine_commitments AS (
			 ...
			 SELECT
			   ...
			   WHERE organization_id = $1 AND deleted_at IS NULL
			     AND expires_at > statement_timestamp()), 0)::text AS storage_byte_seconds`, [organizationId]);
        ...
        projectedMicros += ceilingPrice(durable.memory_mebibyte_seconds, this.#rate('gib_seconds'), mebibytesPerGibibyte);
        projectedMicros += ceilingPrice(durable.storage_byte_seconds, this.#rate('storage_gib_hours'), bytesPerGibibyteHour);
        projectedMicros += ceilingPrice(additional.vcpuSeconds ?? 0n, this.#rate('vcpu_seconds'));
        projectedMicros += ceilingPrice(additional.memoryMebibyteSeconds ?? 0n, this.#rate('gib_seconds'), mebibytesPerGibibyte);
        projectedMicros += ceilingPrice(additional.storageByteSeconds ?? 0n, this.#rate('storage_gib_hours'), bytesPerGibibyteHour);
        if (projectedMicros > BigInt(account.spend_cap_cents) * microsPerCent) {
            throw new BillingAdmissionRejected('spend_cap_exceeded', 'Organization spend cap would be exceeded by this resource commitment.');
        }
    }
    #rate(dimension) {
