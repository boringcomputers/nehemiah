...
const bytesPerGibibyte = 1073741824n;
...
const managedEgressBytesPerSecond = 8000000n;
const managedEgressBurstBytes = 1048576n;
...
const receiptFor = (observation, outcome) => ({
...
const observationIntegrityReason = (identity, state, observation, receivedAt) => {
    ...
    const defensibleElapsedNs = BigInt(Math.max(0, receivedAt.getTime() - leaseStart.getTime()) + maximumWallDivergenceMs) *
        nanosecondsPerMillisecond;
    if (runtime > defensibleElapsedNs)
        return 'observation_rate_exceeded';
    const maximumCumulativeEgress = managedEgressBurstBytes +
        (defensibleElapsedNs * managedEgressBytesPerSecond + nanosecondsPerSecond - 1n) /
            nanosecondsPerSecond;
    if (egress > maximumCumulativeEgress)
        return 'egress_plausibility_exceeded';
    if (!state.start_seen)
        return undefined;
    const priorRuntime = BigInt(state.last_runtime_ns);
    const priorEgress = BigInt(state.last_egress_bytes);
    if (runtime < priorRuntime || egress < priorEgress)
        return undefined;
...
const parseUnsigned = (value, name) => {
    if (!unsignedInteger.test(value))
        throw new MeteringInputError(`${name} must be an unsigned decimal integer`);
    const parsed = BigInt(value);
    if (parsed > maxUnsignedBigint)
        throw new MeteringInputError(`${name} exceeds host uint64`);
    return parsed;
};
const parsePositiveBigint = (value, name) => {
    ...
    const parsed = BigInt(value);
    if (parsed > maxSignedBigint)
        throw new MeteringInputError(`${name} exceeds PostgreSQL bigint`);
    return parsed;
};
const canonicalPayload = (observation) => JSON.stringify({
...
const payloadHash = (observation) => createHash('sha256').update(canonicalPayload(observation)).digest();
const validateObservation = (observation) => {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(observation.leaseId)) {
        throw new MeteringInputError('lease_id is invalid');
    }
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(observation.hostBootId)) {
    ...
    if ((observation.quality === 'exact' && observation.qualityReason !== 'none') ||
    ...
    if (!Number.isFinite(observedAt.getTime()) ||
        !/^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]{1,9})?Z$/.test(observation.observedAt)) {
        throw new MeteringInputError('observed_at must be a canonical UTC timestamp');
    }
};
const timestampFromNanoseconds = (value) => new Date(Number(value / nanosecondsPerMillisecond));
/** Split a monotonic interval at UTC midnights without losing a nanosecond. */
export const splitRuntimeAtUtcMidnight = (start, durationNs) => {
    if (durationNs < 0n)
        throw new Error('runtime delta cannot be negative');
    if (durationNs === 0n)
        return [];
    ...
    while (cursor < terminal) {
        ...
        const nextMidnight = BigInt(Date.UTC(cursorDate.getUTCFullYear(), cursorDate.getUTCMonth(), cursorDate.getUTCDate() + 1)) * nanosecondsPerMillisecond;
...
const exception = async (client, input) => {
    await client.query(`INSERT INTO metering_exceptions
...
const stateForUpdate = async (client, machineId) => (await client.query(`SELECT * FROM machine_meter_state WHERE machine_id = $1 FOR UPDATE`, [machineId])).rows[0];
const setQuarantined = async (client, machineId, reason) => {
    await client.query(`UPDATE machine_meter_state
		 SET quarantined_at = COALESCE(quarantined_at, now()),
		     quarantine_reason = COALESCE(quarantine_reason, $2), updated_at = now()
		 WHERE machine_id = $1`, [machineId, reason]);
};
const quarantineHost = async (client, hostId, reason) => {
    await client.query(`UPDATE hosts
		 SET desired_state = 'quarantined', state = 'stale',
		     ...
		     updated_at = now()
		 WHERE id = $1 AND desired_state NOT IN ('quarantined', 'revoked')`, [hostId, reason]);
    await client.query(`INSERT INTO audit_events
		 ...
		 ON CONFLICT (event_key) DO NOTHING`, [`host:${hostId}:metering-integrity-quarantine`, hostId, reason]);
};
const appendProjection = async (client, input) => {
    if (input.quantity === 0n)
        return;
    await client.query(`INSERT INTO meter_raw_usage_events
    ...
    const projection = input.unit === 'vcpu_nanosecond'
...
const closeLegacyPrefix = async (client, identity, hostStart) => {
    const legacyStart = identity.usage_checkpoint_at ?? identity.started_at;
    const boundary = legacyStart && legacyStart.getTime() > hostStart.getTime() ? legacyStart : hostStart;
    if (legacyStart && legacyStart.getTime() < boundary.getTime()) {
        await client.query(`INSERT INTO usage_outbox
            ...
            identity.id,
            identity.vcpus,
            Number(BigInt(identity.memory_bytes) / 1048576n),
            legacyStart,
            boundary
        ]);
    }
    await client.query(`UPDATE machines SET usage_checkpoint_at = GREATEST(COALESCE(usage_checkpoint_at, $2), $2)
		 WHERE id = $1 AND lease_id = $3 AND lease_generation = $4::bigint`, [identity.id, boundary, identity.lease_id, identity.lease_generation]);
    return boundary;
};
const processObservation = async (client, identity, state, observation) => {
    ...
    if (state.quarantined_at)
        anomaly = state.quarantine_reason;
    else if (state.host_boot_id !== observation.host_boot_id)
        anomaly = 'boot_id_changed';
    else if (state.loss_closed_at)
        anomaly = 'observation_after_final';
    else if (!state.start_seen &&
        (sequence !== 1n || observation.kind !== 'start' || runtime !== 0n)) {
        anomaly = 'start_missing';
    }
    else if (state.start_seen && observation.kind === 'start')
        anomaly = 'start_repeated';
    else if (state.final_seen)
        anomaly = 'observation_after_final';
    else if (runtime < priorRuntime ||
        egress < priorEgress ||
    ...
    if (!anomaly &&
        ...
        if (Math.abs(wallDeltaMs - runtimeDeltaMs) > maximumWallDivergenceMs)
            anomaly = 'clock_skew';
    }
    if (anomaly) {
        ...
        await client.query(`UPDATE machine_meter_state
			 ...
			 WHERE machine_id = $1 AND last_sequence = $4`, [identity.id, observation.sequence, observation.received_at, priorSequence.toString()]);
        return 'quarantined';
    }
    if (observation.kind === 'start') {
        const cutoverBoundary = await closeLegacyPrefix(client, identity, observation.observed_at);
        const legacyCoveredRuntime = BigInt(Math.max(0, cutoverBoundary.getTime() - observation.observed_at.getTime())) *
            nanosecondsPerMillisecond;
        await client.query(`UPDATE machine_meter_state
    ...
    if (!periodStart)
        throw new Error('accepted meter state is missing its period boundary');
    ...
    const periodEnd = timestampFromNanoseconds(BigInt(periodStart.getTime()) * nanosecondsPerMillisecond + runtimeDelta);
    await client.query(`UPDATE machine_meter_state
    ...
    if (observation.kind === 'final') {
        await client.query(`UPDATE machines
			 SET usage_checkpoint_at = $2, usage_finalized_at = COALESCE(usage_finalized_at, now())
			 WHERE id = $1 AND lease_id = $3 AND lease_generation = $4`, [identity.id, periodEnd, identity.lease_id, identity.lease_generation]);
...
export class AuthoritativeMetering {
    database;
    minimumBatchIntervalMs;
    constructor(database, minimumBatchIntervalMs = minimumMeteringBatchIntervalMs) {
        this.database = database;
        this.minimumBatchIntervalMs = minimumBatchIntervalMs;
        if (!Number.isSafeInteger(minimumBatchIntervalMs) ||
    ...
    async ingest(hostId, observations, credentialGeneration) {
        ...
        return this.database.transaction(async (client) => {
            const host = await client.query(`SELECT desired_state::text, credential_status::text, credential_generation,
				 ...
				 FROM hosts
				 WHERE id = $1 FOR UPDATE`, [hostId, this.minimumBatchIntervalMs]);
            if (!host.rows[0] ||
            ...
            await client.query('UPDATE hosts SET last_metering_batch_at = statement_timestamp() WHERE id = $1', [hostId]);
            const receipts = [];
            ...
            for (const input of observations) {
                ...
                const identity = (await client.query(`SELECT id, organization_id, project_id, host_id, host_machine_id,
						 ...
						 FOR UPDATE`, [input.machineId, hostId, input.hostMachineId, input.leaseId, input.leaseGeneration])).rows[0];
                if (!identity ||
                    identity.vcpus !== input.vcpus ||
                    identity.memory_bytes !== input.memoryBytes) {
                    throw new MeteringInputError('observation does not match the authoritative machine lease and resources');
                }
                let state = await stateForUpdate(client, input.machineId);
                if (!state) {
                    await client.query(`INSERT INTO machine_meter_state
                ...
                let sequenceViolation = state.host_boot_id !== input.hostBootId ||
                    sequence > lastSequence + maximumForwardSequenceWindow;
                if (!sequenceViolation && sequence > lastSequence + 1n) {
                    const pending = await client.query(`SELECT count(*)::text AS count
						 ...
						 WHERE observation.host_id = $1
						   AND observation.sequence > meter.last_sequence`, [hostId]);
                    sequenceViolation =
                ...
                const integrityViolation = sequence === lastSequence + 1n
                    ? observationIntegrityReason(identity, state, {
                        kind: input.kind,
                ...
                const inserted = await client.query(`INSERT INTO host_usage_observations
                ...
                if (!stored) {
                    duplicate = true;
                    stored = (await client.query(`SELECT id::text, payload_hash, host_boot_id::text, sequence::text, kind,
								 quality, quality_reason,
								 runtime_ns::text, egress_bytes::text, egress_counter_epoch::text,
								 observed_monotonic_ns::text,
							 observed_at, received_at, vcpus, memory_bytes::text
							 ...
							 WHERE host_id = $1 AND host_boot_id = $2 AND lease_id = $3
							   AND lease_generation = $4::bigint AND sequence = $5::bigint`, [hostId, input.hostBootId, input.leaseId, input.leaseGeneration, input.sequence])).rows[0];
                ...
                for (;;) {
                    ...
                    const pending = (await client.query(`SELECT id::text, payload_hash, host_boot_id::text, sequence::text, kind,
								 quality, quality_reason,
								 runtime_ns::text, egress_bytes::text, egress_counter_epoch::text,
								 observed_monotonic_ns::text,
							 observed_at, received_at, vcpus, memory_bytes::text
							 ...
							 WHERE machine_id = $1 AND lease_generation = $2::bigint
							   AND host_boot_id = $3 AND sequence = $4::bigint`, [identity.id, identity.lease_generation, state.host_boot_id, next])).rows[0];
                    if (!pending)
                        break;
                ...
                await client.query(`INSERT INTO metering_exception_resolutions (exception_id, resolution)
					 ...
					 ON CONFLICT (exception_id) DO NOTHING`, [identity.id, identity.lease_generation, state.last_sequence]);
                receipts.push(receiptFor(input, inputQuarantined
                    ? 'quarantined'
                    : processedInput
                        ? duplicate
                            ? 'duplicate'
    ...
    async closeHostLoss(machineId, leaseId) {
        return this.database.transaction(async (client) => {
            const machine = (await client.query(`SELECT m.host_id, m.lease_generation::text,
					 ...
					 WHERE m.id = $1 AND m.lease_id = $2 FOR UPDATE OF m`, [machineId, leaseId])).rows[0];
            if (!machine)
                return undefined;
            if (!machine.meter_state)
                return undefined;
            ...
            if (!machine.final_seen) {
                ...
                await client.query(`UPDATE machine_meter_state
					 ...
					 WHERE machine_id = $1 AND lease_id = $2`, [machineId, leaseId]);
            }
            await client.query(`UPDATE machines
				 SET usage_checkpoint_at = COALESCE($3, usage_checkpoint_at),
				     usage_finalized_at = COALESCE(usage_finalized_at, now())
				 WHERE id = $1 AND lease_id = $2`, [machineId, leaseId, machine.last_period_end]);
