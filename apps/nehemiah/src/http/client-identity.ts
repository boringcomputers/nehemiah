...
const signaturePayload = (address: string, timestamp: string): string =>
	`nehemiah-client-address-v1\n${timestamp}\n${address}`;

const canonicalAddress = (raw: string | undefined): string | undefined => {
	const value = (raw ?? '').trim().replace(/^::ffff:(?=\d+\.\d+\.\d+\.\d+$)/i, '');
	if (value.includes(',') || isIP(value) === 0) return undefined;
 ...
 */
export const verifiedClientAddress = (
	headers: Headers,
	...
	if (!forwarded || !timestamp || !signature || !/^[0-9]{1,12}$/.test(timestamp)) return direct;
	...
	if (!/^[a-f0-9]{64}$/.test(signature)) return direct;
