...
const dataPlaneErrorResponse = (description: string) => ({
	description,
	content: {
...
const websocketErrorResponse = (description: string) => ({
	description,
	content: {
		'application/problem+json': { schema: schemaRef('Problem') },
		'application/json': { schema: schemaRef('DataPlaneError') },
...
const errors = (...statuses: ReadonlyArray<keyof typeof errorDescriptions>) =>
	Object.fromEntries(
		statuses.map((status) => [
			String(status),
...
const websocketErrors = (...statuses: ReadonlyArray<keyof typeof errorDescriptions>) =>
	Object.fromEntries(
		statuses.map((status) => [
			String(status),
...
const gatewayErrors = (...statuses: ReadonlyArray<keyof typeof errorDescriptions>) =>
	Object.fromEntries(
		statuses.map((status) => [
			String(status),
			status === 429
...
const proxiedDataPlaneErrors = (...statuses: ReadonlyArray<502 | 503>) =>
	Object.fromEntries(
		statuses.map((status) => [
			String(status),
			status === 503
...
const managedVolumeDisabledDescription =
	'Managed volume and object transfer operations are production-disabled until bounded volume/revision counts and global transfer quotas are enforced. Production returns typed 503 without reading, reserving, or writing volume data.';
...
const websocketSwitchingProtocols = {
	description:
		'WebSocket connection established; subsequent messages are protocol-specific frames.',
	headers: {
...
const websocketContract = ({
	...
	readonly capability: 'tty' | 'vnc' | 'agent';
	readonly clientFrames: ReadonlyArray<'binary' | 'text'>;
	readonly serverFrames: ReadonlyArray<'binary' | 'text'>;
	readonly maxClientFrameBytes: number;
	readonly maxLifetimeSeconds?: number;
	readonly initialFrame?: Readonly<Record<string, unknown>>;
}) => ({
	capability,
	...
	idle: {
		timeout_seconds: 90,
		behavior:
			'Gateway/host pings require a timely pong; protocol activity refreshes applicable reads.'
	},
	lifetime: {
		maximum_seconds: maxLifetimeSeconds,
		behavior:
			'Closes at the earliest of capability expiry, current machine-lease expiry, gateway stream limit, or this protocol limit.'
	},
	...(initialFrame === undefined ? {} : { initial_client_frame: initialFrame }),
	payload_modeling:
		'Message payloads are protocol-specific frames and are not JSON request/response bodies modeled by OpenAPI.'
});
...
export const openApiDocument = {
	...
	info: {
		...
		description:
			'Public Boring Computers control-plane and capability-gateway HTTP contract. WebSocket byte channels are documented separately.'
	},
	...
	components: {
		securitySchemes: {
			...
			websocketCapability: {
				...
				description:
					'Short-lived machine capability prefixed with nehemiah.capability.; credentials in the URL are forbidden.'
			},
			volumeCapability: {
				type: 'apiKey',
				in: 'header',
		...
		schemas: {
			...
			IdentityProviderSyncRequest: {
				type: 'object',
				description:
					'A bounded, source-ordered Clerk lifecycle event. User events forbid organization_id/role; membership.upserted requires both; membership.removed requires organization_id and forbids role.',
				required: [
					'provider',
					'event_id',
					'source_version',
			...
			DeviceCodeResponse: {
				...
				properties: {
					device_code: {
						type: 'string',
						pattern:
							'^bc_device_[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[A-Za-z0-9_-]{43}$',
						readOnly: true
					},
			...
			RefreshDeviceTokenRequest: {
				...
				properties: {
					refresh_token: {
						type: 'string',
						pattern:
							'^bc_refresh_[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[A-Za-z0-9_-]{43}$',
						writeOnly: true
					}
				},
				additionalProperties: false
			...
			DeviceTokenResponse: {
				...
				properties: {
					...
					access_token: {
						type: 'string',
						pattern:
							'^bc_access_[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[A-Za-z0-9_-]{43}$',
						readOnly: true
					},
			...
			RuntimeCohort: {
				...
				additionalProperties: false,
				description:
					'Exact signed managed-host runtime cohort. id is SHA-256 of the canonical contract-version, architecture, built-in rootfs, kernel, Firecracker, and jailer digest lines.'
			},
			MachineResources: {
				type: 'object',
			...
			NetworkPolicyDeclaration: {
				...
				required: ['mode'],
				description:
					'Managed beta is fail-closed to mode=off until aggregate organization, project, and host-network traffic quotas exist.',
				properties: {
			...
			GatewayCapability: {
				...
				description:
					'Gateway capability. The agent value is reserved for local/self-hosted compatibility and receives typed 501 not_supported from managed session issuance.'
			},
			...
			HostEnrollmentResponse: {
				...
				properties: {
					...
					token: {
						...
						readOnly: true,
						description:
							'256-bit one-use host enrollment grant. Store only in the private per-host provisioning input.'
					},
					expires_at: { type: 'string', format: 'date-time' },
					secret_displayed_once: { type: 'boolean', const: true }
				},
	...
	paths: {
		'/v1/auth/device/code': {
			post: {
				...
				responses: {
					'201': jsonResponse(
						'Short-lived device and human authorization codes',
						'DeviceCodeResponse'
					),
		...
		'/v1/auth/device/inspect': {
			post: {
				...
				responses: {
					'200': jsonResponse(
						'Pending requested scopes for explicit review',
						'DeviceAuthorizationRequest'
					),
		...
		'/v1/auth/device/authorize': {
			post: {
				...
				responses: {
					'200': jsonResponse(
						'One-time approval or denial recorded',
						'DeviceAuthorizationDecision'
					),
		...
		'/v1/auth/device/token': {
			post: {
				...
				responses: {
					'200': jsonResponse(
						'One-time access and refresh credential response',
						'DeviceTokenResponse'
					),
					...errors(400, 413, 429, 500)
				}
		...
		'/v1/auth/device/refresh': {
			post: {
				...
				responses: {
					'200': jsonResponse(
						'Rotated refresh credential and short-lived access token',
						'DeviceTokenResponse'
					),
		...
		'/v1/operator/identity-provider/sync': {
			post: {
				...
				description:
					'Explicit fleet-operator Clerk lifecycle synchronization. Events map only to existing immutable local subjects and explicit local organization UUIDs; raw provider payloads and credentials are never retained.',
				requestBody: jsonRequest('IdentityProviderSyncRequest'),
				responses: {
					'200': jsonResponse(
						'Applied, stale, or replayed identity-provider event',
						'IdentityProviderSyncResponse'
					),
					...errors(400, 401, 403, 404, 409, 413, 429, 500, 503)
				}
		...
		'/v1/operator/organizations/{id}/disable': {
			...
			post: {
				...
				responses: {
					'200': jsonResponse(
						'Disabled organization identity state',
						'OrganizationIdentityLifecycleResponse'
					),
					...errors(400, 401, 403, 404, 413, 429, 500, 503)
				}
		...
		'/v1/operator/organizations/{id}/enable': {
			...
			post: {
				...
				responses: {
					'200': jsonResponse(
						'Enabled organization identity state',
						'OrganizationIdentityLifecycleResponse'
					),
					...errors(400, 401, 403, 404, 413, 429, 500, 503)
				}
		...
		'/v1/machines/{id}/sessions': {
			...
			post: {
				...
				description:
					'Issues a revocable managed gateway session. The host-local agent capability is deliberately unsupported and returns typed 501 before grant persistence.',
		...
		'/v1/machines/{id}/agent': {
			...
			get: {
				...
				deprecated: true,
				description:
					'Local/self-hosted compatibility route only. Managed cloud does not issue the required agent capability or distribute a provider model credential.',
				security: [{ websocketCapability: [] }],
				parameters: websocketUpgradeParameters,
				responses: {
					'101': websocketSwitchingProtocols,
				...
				'x-nehemiah-websocket': websocketContract({
					...
					initialFrame: {
						...
						deadline_seconds: 5,
						format:
							'Exact UTF-8 JSON object {"type":"start","version":1,"goal":"..."}; unknown fields are rejected.',
			...
			get: {
				...
				parameters: [
					...websocketUpgradeParameters,
					{
						...
						description:
							'Legacy local/self-hosted compatibility only. Managed clients MUST omit this query and send the start frame after upgrade.'
					}
				],
				responses: {
					'101': websocketSwitchingProtocols,
				...
				responses: {
					'204': {
						...
						headers: {
							'Set-Cookie': {
								description:
									'Short-lived Secure, SameSite=Strict preview cookie scoped to the isolated preview origin',
								schema: { type: 'string' }
							}
						}
					},
		...
		'/v1/templates': {
			...
			post: {
				...
				deprecated: true,
				description:
					'Managed custom-template publication is disabled in production until aggregate tenant/host-cache quotas and durable eviction are enforced. Production returns 503 without exporting or persisting a template.',
		...
		'/v1/operator/host-enrollments': {
			post: {
				...
				description:
					'Fleet-operator-only issuance of a short-lived, one-use grant bound to one provider identity, overlay address, architecture, and capacity.',
				requestBody: jsonRequest('CreateHostEnrollmentRequest'),
				responses: {
					'201': jsonResponse(
						'One-time host enrollment grant; the token is displayed once',
						'HostEnrollmentResponse'
					),
		...
		'/v1/operator/hosts/{id}/credentials/rotate': {
			...
			post: {
				...
				responses: {
					'200': jsonResponse(
						'Rotated one-time host credential and lifecycle state',
						'RotateHostCredentialsResponse'
					),
					...errors(400, 401, 403, 409, 413, 429, 500, 503)
				}
...
export const openapi: Handler = () => json(openApiDocument);
