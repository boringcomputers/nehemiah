...
import {
	...
	InvalidIdentityLifecycleRequest
} from '../../domain/identity-lifecycle.js';
import type {
...
import { HttpRequestError, json, problem, readJson, type Handler, type Router } from '../router.js';
...
const syncEvents: ReadonlyArray<IdentityProviderSyncEventType> = [
	'user.disabled',
	'user.deleted',
	'membership.upserted',
	'membership.removed'
];
const membershipRoles: ReadonlyArray<IdentityProviderMembershipRole> = [
	'owner',
	'admin',
	'member',
	'billing'
...
interface ParsedIdentityProviderSync {
	readonly input: IdentityProviderSyncInput;
	readonly reason: string;
}

const parseIdentityProviderSync = (value: unknown): ParsedIdentityProviderSync => {
	...
	if (
		typeof body.source_version !== 'number' ||
	...
	if (
		...
		!syncEvents.includes(body.event_type as IdentityProviderSyncEventType)
	) {
	...
	const eventType = body.event_type as IdentityProviderSyncEventType;
	...
	if (
		...
		throw new InvalidIdentityLifecycleRequest(
			'The identity provider sync body contains missing or unsupported fields.'
		);
	}
	if (
		membershipEvent &&
		...
		throw new InvalidIdentityLifecycleRequest(
			'membership events require a valid local organization UUID'
		);
	}
	if (
		eventType === 'membership.upserted' &&
		(typeof body.role !== 'string' ||
			!membershipRoles.includes(body.role as IdentityProviderMembershipRole))
	) {
	...
	return {
		input: {
			...
			...(membershipEvent ? { organizationId: body.organization_id as string } : {}),
			...(eventType === 'membership.upserted'
				? { role: body.role as IdentityProviderMembershipRole }
				: {})
		},
...
const reason = async (request: Request, requestId: string): Promise<string | Response> => {
	try {
		...
		if (
			...
			return problem(
				400,
				'invalid_identity_lifecycle_request',
				'A reason containing 1 to 512 characters is required.',
				requestId
			);
		}
		...
		return problem(
			400,
			'invalid_identity_lifecycle_request',
			'The identity lifecycle request is invalid.',
			requestId
		);
	}
};

export const registerIdentityLifecycleRoutes = <S extends IdentityLifecycleRouteServices>(
	router: Router<S>
): void => {
	const lifecycle =
		(target: 'users' | 'organizations', transition: 'disable' | 'enable'): Handler<S> =>
		async ({ request, params, services, requestId }) => {
			...
			if (!uuid.test(params.id!)) {
				return problem(
					400,
					'invalid_identity_lifecycle_request',
					'A valid target identity ID is required.',
					requestId
				);
			}
			const lifecycleReason = await reason(request, requestId);
			if (lifecycleReason instanceof Response) return lifecycleReason;
			const context = {
	...
	router.post('/v1/operator/identity-provider/sync', async ({ request, services, requestId }) => {
		...
		let parsed: ParsedIdentityProviderSync;
		try {
			parsed = parseIdentityProviderSync(await readJson<unknown>(request, 16_384));
		} catch (error) {
			await services.audit.record({
				organizationId: principal.organizationId,
				actorType: 'user',
				...
				outcome: 'denied',
				reasonCode:
					error instanceof HttpRequestError ? error.code : 'invalid_identity_provider_sync',
				resourceType: 'identity_provider_event',
				resourceId: 'unparsed'
			});
			if (error instanceof HttpRequestError) throw error;
