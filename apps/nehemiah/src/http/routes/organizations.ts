...
import { authenticate, authenticateIdentity, canAdminister, type AuthServices } from '../auth.js';
...
export const registerOrganizationRoutes = <S extends OrganizationRouteServices>(
	router: Router<S>
): void => {
	router.get('/v1/organizations', async ({ request, services, requestId }) => {
		...
		if (!identity || identity.kind !== 'user') {
			return problem(
				...
				'A dashboard session is required.',
				requestId
			);
		}
	...
	router.get('/v1/projects', async ({ request, services, requestId }) => {
		...
		if (!principal) return problem(401, 'unauthorized', 'A valid session is required.', requestId);
		return json({
			projects: await services.projects.list(
				principal.organizationId,
				principal.kind === 'api_key' ? principal.projectId : undefined
			)
		});
	});
	...
	router.post('/v1/projects', async ({ request, services, requestId }) => {
		...
		try {
			...
			return problem(
				400,
				'invalid_project',
				error instanceof SyntaxError ? error.message : 'The request body is invalid.',
				requestId
			);
		}
		if (
		...
		let creation: ProjectCreateResult;
		try {
