...
export const registerTemplateRoutes = <S extends TemplateRouteServices>(
	router: Router<S>
): void => {
	router.post('/v1/templates', async ({ request, services, requestId }) => {
		...
		if (
			!permits(principal, 'templates:write') ||
			(principal.kind === 'user' && !canAdminister(principal))
		) {
			return problem(
				403,
				'insufficient_scope',
				'templates:write and an administrator role are required.',
				requestId
			);
		}
		try {
			...
			if (
				...
				Object.keys(body).some(
					(key) => !['project_id', 'machine_id', 'name', 'version'].includes(key)
				)
			) {
				return problem(
					400,
					'invalid_template_request',
					'Only project_id, machine_id, name, and version are accepted.',
					requestId
				);
			}
			...
			if (
				...
				return problem(
					400,
					'invalid_template_request',
					'machine_id, name, and version are required.',
					requestId
				);
			}
	...
	router.get('/v1/templates', async ({ request, url, services, requestId }) => {
		...
		return json({
			templates: (await services.templates.list(principal.organizationId, projectId)).map(
				templateJson
			)
		});
	});
	...
	router.delete('/v1/templates/:id', async ({ request, url, params, services, requestId }) => {
		...
		if (result === 'in_use') {
			return problem(
				409,
				'template_in_use',
				'Running machines still reference this immutable template version.',
				requestId
			);
		}
