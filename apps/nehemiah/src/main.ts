...
import { healthz, readyz, type HealthServices } from './http/health.js';
import {
...
} from './http/incoming-request.js';
import { openapi } from './http/openapi.js';
import { HttpRequestError, problem, Router } from './http/router.js';
import { registerApiKeyRoutes, type ApiKeyRouteServices } from './http/routes/api-keys.js';
import { registerBillingRoutes, type BillingRouteServices } from './http/routes/billing.js';
import {
	registerDeviceAuthorizationRoutes,
	type DeviceAuthorizationRouteServices
} from './http/routes/device-authorization.js';
import {
	registerInternalHostRoutes,
	type InternalRouteServices
} from './http/routes/internal-hosts.js';
import {
	registerIdentityLifecycleRoutes,
	type IdentityLifecycleRouteServices
} from './http/routes/identity-lifecycle.js';
import { registerMachineRoutes, type MachineRouteServices } from './http/routes/machines.js';
import {
	registerOrganizationRoutes,
	type OrganizationRouteServices
} from './http/routes/organizations.js';
import {
	registerStripeWebhookRoute,
	type StripeRouteServices
} from './http/routes/stripe-webhook.js';
import { registerTemplateRoutes, type TemplateRouteServices } from './http/routes/templates.js';
import { registerVolumeRoutes, type VolumeRouteServices } from './http/routes/volumes.js';
...
import {
	PostgresTemplateReplicaRepository,
	TemplateReplicationJob
} from './jobs/replicate-template.js';
...
import {
	...
	registerMeteringMetrics,
	requestId,
	withControlPlaneRequestTelemetry,
	withJobTelemetry
} from './telemetry.js';

export type AppServices = HealthServices &
	MachineRouteServices &
	ApiKeyRouteServices &
	IdentityLifecycleRouteServices &
	InternalRouteServices &
	BillingRouteServices &
	StripeRouteServices &
	OrganizationRouteServices &
	DeviceAuthorizationRouteServices &
...
export const buildRouter = (): Router<AppServices> => {
	const router = new Router<AppServices>();
...
const start = async (): Promise<void> => {
	...
	const deviceAuthorizations = new DeviceAuthorizationService(
		database,
		config.deviceVerificationUrl,
		Redacted.value(config.deviceCodePepper)
	);
	...
	const volumeStorage =
		config.objectStorage && config.volumeBroker
	...
	const hosts = new HostService(
		database,
		hostCredentialCipher,
		config.hostCidrs,
		config.telemetry?.serviceVersion
	);
	const identityLifecycle = new IdentityLifecycleService(database);
	const hostClient = new NehemiahdClient(
		new PostgresHostCredentialResolver(database, hostCredentialCipher),
		fetch,
		30_000,
		config.hostPort
	);
	const templateTransferClient =
		config.templateTransfers && config.objectStorage
			? new NehemiahdTemplateClient(
					new PostgresHostCredentialResolver(database, hostCredentialCipher),
					...
					fetch,
					config.templateTransfers.hostTimeoutMs,
					config.hostPort
				)
			: undefined;
	...
	const templates = new TemplateService(
		...
		machines,
		templateStorage,
		templateTransferClient
	);
	const templateReplicationWorker = templateTransferClient
		? new TemplateReplicationJob(
				new PostgresTemplateReplicaRepository(database),
				templateStorage,
				templateTransferClient
			)
		: undefined;
	...
	const fleetTimer = setInterval(
		() => {
			void shutdown.runJob(() =>
				withJobTelemetry('host_stale_marker', () =>
					markStaleHosts(hosts, config.hostStaleAfterMs)
				).catch((error) => log('error', 'host stale-marker failed', { error }))
			);
		},
	...
	const reconcileTimer = setInterval(() => {
		void shutdown.runJob(() =>
			withJobTelemetry('machine_reconciliation', () => reconciler.run()).catch((error) =>
				log('error', 'machine reconciliation failed', { error })
			)
		);
	}, 10_000);
	const reapTimer = setInterval(() => {
		void shutdown.runJob(() =>
			withJobTelemetry('machine_expiry', () => reapExpiredMachines(database, machines)).catch(
				(error) => log('error', 'machine expiry reaper failed', { error })
			)
		);
	}, 15_000);
	const gatewayGrantReapTimer = setInterval(() => {
		void shutdown.runJob(() =>
			withJobTelemetry('gateway_grant_expiry', () => reapExpiredGatewayGrants(database)).catch(
				(error) => log('error', 'gateway grant expiry reaper failed', { error })
			)
		);
	}, 60_000);
	const deviceAuthorizationReapTimer = setInterval(() => {
		void shutdown.runJob(() =>
			withJobTelemetry('device_authorization_retention', () =>
				reapExpiredDeviceAuthorizations(database)
			).catch((error) => log('error', 'device authorization retention failed', { error }))
		);
	}, 60_000);
	const gatewayStreamReapTimer = setInterval(() => {
		void shutdown.runJob(() =>
			withJobTelemetry('gateway_stream_expiry', () => streamAdmission.reapExpired()).catch(
				(error) => log('error', 'gateway stream lease expiry failed', { error })
			)
		);
	}, 15_000);
	const usageTimer = setInterval(
		() => {
			void shutdown.runJob(() =>
				withJobTelemetry('usage_aggregation', () => aggregateUsage(database)).catch((error) =>
	...
	const volumeDeletionTimer = volumeDeletionWorker
		? setInterval(() => {
				void shutdown.runJob(() =>
					withJobTelemetry('volume_deletion', () => volumeDeletionWorker.run()).catch((error) =>
						log('error', 'volume deletion scheduling failed', { error })
					)
				);
			}, 15_000)
		: undefined;
	const templateReplicationTimer =
		templateReplicationWorker && config.templateTransfers
			? setInterval(() => {
					void shutdown.runJob(() =>
						withJobTelemetry('template_replication', () =>
							templateReplicationWorker.runOnce()
						).catch((error) => log('error', 'template replication scheduling failed', { error }))
					);
	...
	const server = createServer(controlPlaneHttpServerOptions, (request, response) => {
		void shutdown.runRequest(
			async () => {
				const started = performance.now();
				const pendingBody = admitPendingRequestBody(
					request,
					response,
					Redacted.value(config.gatewayToken),
					pendingRequestBodies
				);
				if (!pendingBody) {
	...
	const initiateShutdown = (): Promise<void> =>
		shutdown.shutdown({
			...
			onError: (phase, error) =>
				log('error', 'control plane shutdown phase failed', { reason: phase, error })
		});
	const onSignal = (): void => {
