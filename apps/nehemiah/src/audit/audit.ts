...
const safeToken = (value: string, fallback: string): string =>
	/^[a-z][a-z0-9._:-]{0,127}$/.test(value) ? value : fallback;

const classReason = (error: Error): string =>
	error.constructor.name
		.replace(/([a-z0-9])([A-Z])/g, '$1_$2')
		.replace(/[^A-Za-z0-9._:-]/g, '_')
		.toLowerCase();

export const auditFailureReason = (error: unknown): string => {
	if (typeof error === 'object' && error !== null && 'code' in error) {
		const code = Reflect.get(error, 'code');
		if (typeof code === 'string') return safeToken(code.toLowerCase(), 'operation_failed');
	}
...
export const auditUserAgentFingerprint = (value: string | undefined): string | null =>
	value ? `sha256:${createHash('sha256').update(value, 'utf8').digest('hex')}` : null;
...
export class AuditService {
	...
	async record(input: AuditRecord): Promise<{ eventKey: string; operationId: string }> {
	 ...
	 */
	async authentication(input: AuthenticationAuditRecord): Promise<void> {
		...
		const slot = createHash('sha256')
			...
			.readUInt16BE(0);
		await this.database.query(
			`WITH current_window(window_started_at) AS (
			   SELECT to_timestamp(
	...
	async capture<T>(input: AuditOperation<T>, operation: () => Promise<T>): Promise<T> {
		...
		try {
			...
			if (input.failureHandled?.(error)) throw error;
			try {
				await this.record({
					...base,
					outcome: 'failed',
		...
		if (completion?.deferTerminal) return result;
		try {
			await this.record({
				...base,
