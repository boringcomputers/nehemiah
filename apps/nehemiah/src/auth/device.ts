...
import { apiKeyScopes, type ApiKeyPrincipal, type ApiKeyScope } from './api-key.js';
...
const throwDeviceRetentionCapacity = (error: unknown): never => {
	if (
		typeof error === 'object' &&
		error !== null &&
		...
		throw new DeviceAuthorizationError(
			'rate_limited',
			'Device login capacity is temporarily exhausted.',
			429,
			60
		);
	}
...
interface DeviceRow {
	readonly id: string;
	readonly device_token_hash: Buffer;
	readonly client_id: string;
	readonly requested_scopes: ApiKeyScope[];
	readonly approved_scopes: ApiKeyScope[] | null;
...
interface RefreshRow {
	...
	readonly user_disabled_at: Date | null;
	readonly organization_disabled_at: Date | null;
}
...
const equalDigest = (expected: Buffer | undefined, raw: string): boolean =>
	timingSafeEqual(expected?.byteLength === 32 ? expected : ZERO_DIGEST, digest(raw)) &&
	expected !== undefined;

const normalizeUserCode = (value: string): string | undefined => {
...
const formatUserCode = (normalized: string): string =>
...
const opaqueToken = (kind: 'device' | 'access' | 'refresh', id: string): string =>
	`bc_${kind}_${id}_${randomBytes(32).toString('base64url')}`;

const tokenId = (raw: string, kind: 'device' | 'access' | 'refresh'): string | undefined => {
	const match = new RegExp(
		`^bc_${kind}_([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})_[A-Za-z0-9_-]{43}$`,
		'i'
	).exec(raw);
	return match?.[1];
};

const validScopes = (scopes: ReadonlyArray<unknown>): scopes is ReadonlyArray<ApiKeyScope> =>
	scopes.length > 0 &&
	scopes.length <= apiKeyScopes.length &&
	scopes.every((scope) => typeof scope === 'string' && apiKeyScopes.includes(scope as ApiKeyScope));

const uniqueScopes = (scopes: ReadonlyArray<ApiKeyScope>): ApiKeyScope[] => [...new Set(scopes)];

const rolePermits = (role: MembershipRole, scope: ApiKeyScope): boolean => {
	if (role === 'owner' || role === 'admin') return true;
	if (role === 'billing') return scope === 'billing:read';
	return (
		scope === 'machines:read' ||
...
const audit = async (
	...
	input: {
		...
		readonly outcome: 'requested' | 'succeeded' | 'denied';
...
const revokeFamilyForIdentity = async (
	client: PoolClient,
	row: Pick<RefreshRow, 'family_id' | 'organization_id' | 'project_id'>,
	now: Date,
	context: { requestId?: string; userAgent?: string }
): Promise<void> => {
	...
	await client.query(
		'UPDATE device_access_tokens SET revoked_at = COALESCE(revoked_at, $2) WHERE family_id = $1',
		[row.family_id, now]
	);
	await client.query(
		...
		[row.family_id, row.organization_id]
	);
	if (revoked.rowCount) {
		await audit(client, {
...
const rateLimit = async (
	client: PoolClient,
	kind: 'issue_source' | 'issue_global' | 'approve_actor',
	keyHash: Buffer,
	limit: number,
	now: Date,
	...
	const result = await client.query<{ attempts: number }>(
		`INSERT INTO device_authorization_rate_limits
...
const cleanupRateLimits = async (client: PoolClient, now: Date): Promise<void> => {
	await client.query(
		`DELETE FROM device_authorization_rate_limits
		 WHERE ctid IN (
...
export class DeviceAuthorizationService {
	...
	#lookupDigest(value: string): Buffer {
		return createHmac('sha256', this.#lookupKey).update(value).digest();
	}

	async #consumeIssueRateLimits(source: string, now: Date): Promise<void> {
		const retryAfter = await this.database.transaction(async (client) => {
			...
			const global = await rateLimit(
				client,
				'issue_global',
				this.#lookupDigest('global'),
				300,
				now,
				60
			);
			const perSource = await rateLimit(
				client,
				'issue_source',
				this.#lookupDigest(source || 'unknown'),
				10,
				now,
				60
			);
	...
	async #consumeActorRateLimit(actorId: string, now: Date): Promise<void> {
		const retryAfter = await this.database.transaction(async (client) => {
			await cleanupRateLimits(client, now);
	...
	async inspect(input: {
		...
		if (!code) throw new DeviceAuthorizationError('invalid_request', 'The user code is invalid.');
		return this.database.transaction(async (client) => {
			const result = await client.query<{
				...
				`SELECT client_id, requested_scopes, expires_at, status
				 FROM device_authorizations WHERE user_code_hash = $1`,
				[this.#lookupDigest(code)]
			);
	...
	async authorize(input: {
		...
		if (
			...
			throw new DeviceAuthorizationError(
				'invalid_request',
				'Approval requires a valid project identifier.'
			);
		}
		await this.database.transaction(async (client) => {
			const membership = await client.query<{ id: string; role: MembershipRole }>(
				`SELECT u.id, om.role
				 FROM users u
				 ...
				 WHERE u.clerk_user_id = $1 AND om.organization_id = $2
				   AND u.disabled_at IS NULL AND organization.disabled_at IS NULL`,
				[input.clerkUserId, input.organizationId]
			);
			...
			const pending = await client.query<{
				...
				`SELECT id, requested_scopes, status, expires_at
				 FROM device_authorizations WHERE user_code_hash = $1 FOR UPDATE`,
				[this.#lookupDigest(code)]
			);
			...
			if (input.decision === 'deny') {
				await client.query(
					`UPDATE device_authorizations
					 ...
					 WHERE id = $1`,
					[authorization.id, input.organizationId, member.id, now]
				);
				await audit(client, {
			...
			if (!input.projectId || !input.scopes || !validScopes(input.scopes)) {
				throw new DeviceAuthorizationError(
					'invalid_scope',
					'Approval requires a project and at least one valid scope.'
				);
			}
			const scopes = uniqueScopes(input.scopes);
			if (
				scopes.some(
					(scope) =>
						!authorization.requested_scopes.includes(scope) || !rolePermits(member.role, scope)
				)
			) {
				throw new DeviceAuthorizationError(
					'invalid_scope',
					'One or more scopes were not requested or are not permitted for this user.',
					403
				);
			}
			...
			if (!project.rowCount) {
				throw new DeviceAuthorizationError(
					'access_denied',
					'The selected project is not available to this organization.',
					403
				);
			}
			await client.query(
				`UPDATE device_authorizations
				 ...
				 WHERE id = $1`,
				[authorization.id, input.organizationId, input.projectId, member.id, scopes, now]
			);
			await audit(client, {
				organizationId: input.organizationId,
	...
	async exchange(
		...
		if (!id) throw new DeviceAuthorizationError('invalid_grant', 'The device code is invalid.');
		const found = await this.database.query<{ device_token_hash: Buffer }>(
			'SELECT device_token_hash FROM device_authorizations WHERE id = $1',
			[id]
		);
		...
		const result = await this.database
			.transaction(async (client) => {
				const locked = await client.query<DeviceRow>(
					`SELECT id, device_token_hash, client_id, requested_scopes, approved_scopes,
				        status, organization_id, project_id, approved_by, expires_at,
				        poll_interval_seconds, next_poll_at
				 FROM device_authorizations WHERE id = $1 FOR UPDATE`,
					[id]
				);
				const row = locked.rows[0];
				if (!row || row.expires_at <= now) return { error: 'expired_token' as const };
				if (row.status === 'denied') return { error: 'access_denied' as const };
				if (row.status === 'consumed') return { error: 'invalid_grant' as const };
				if (row.status === 'pending') {
					if (row.next_poll_at > now) {
						...
						await client.query(
						 ...
						 WHERE id = $1`,
							[id, interval, new Date(now.getTime() + interval * 1_000)]
						);
						return { error: 'slow_down' as const, interval };
					}
					...
					return { error: 'authorization_pending' as const, interval: row.poll_interval_seconds };
				}
				...
				const currentIdentity = await client.query<{
					role: MembershipRole;
				}>(
				 ...
				 WHERE approver.id = $1 AND membership.organization_id = $2
				   AND approver.disabled_at IS NULL AND organization.disabled_at IS NULL`,
				...
				await client.query(
				 ...
				 VALUES ($1, $2, 0, $3, $4, $5)`,
					[refreshId, familyId, digest(refreshToken), now, refreshExpiresAt]
				);
				await client.query(
				...
				await client.query(
					"UPDATE device_authorizations SET status = 'consumed', consumed_at = $2 WHERE id = $1",
					[id, now]
				);
				await audit(client, {
					organizationId: row.organization_id,
	...
	async refresh(
		...
		if (!id) throw new DeviceAuthorizationError('invalid_grant', 'The refresh token is invalid.');
		const now = this.now();
		...
		const outcome = await this.database
			.transaction(async (client) => {
				...
				const familyLock = await client.query<{ family_id: string }>(
				...
				if (!familyLock.rows[0]) return { error: 'invalid_grant' as const };
				const locked = await client.query<RefreshRow>(
					`SELECT rt.id, rt.token_hash, rt.family_id, rt.generation, rt.used_at,
				 ...
				 WHERE rt.id = $1 AND rf.id = $2 FOR UPDATE OF rt`,
					[id, familyLock.rows[0].family_id]
				);
				const row = locked.rows[0];
				...
				if (row.used_at) {
					...
					await client.query(
					 ...
					 SET revoked_at = COALESCE(revoked_at, statement_timestamp())
					 WHERE issuer_type = 'device_family' AND issuer_id = $1
					   AND organization_id = $2 AND expires_at > statement_timestamp()`,
						[row.family_id, row.organization_id]
					);
					...
					return { error: 'refresh_reuse_detected' as const };
				}
				...
				const nextRefreshAt = new Date(
					row.token_created_at.getTime() + MINIMUM_REFRESH_INTERVAL_MS
				);
				if (now < nextRefreshAt) {
					return {
						...
						retryAfterSeconds: Math.max(
							1,
							Math.ceil((nextRefreshAt.getTime() - now.getTime()) / 1_000)
						)
					};
				}
				if (row.generation >= MAXIMUM_REFRESH_GENERATION) {
					await client.query(
						'UPDATE device_refresh_families SET revoked_at = COALESCE(revoked_at, $2) WHERE id = $1',
						[row.family_id, now]
					);
					await client.query(
						'UPDATE device_access_tokens SET revoked_at = COALESCE(revoked_at, $2) WHERE family_id = $1',
				...
				const accessExpiresAt = new Date(
					Math.min(now.getTime() + ACCESS_LIFETIME_MS, row.family_expires_at.getTime())
				);
				await client.query(
					`INSERT INTO device_refresh_tokens
				 (id, family_id, generation, token_hash, created_at, expires_at)
				...
				await client.query(
					'UPDATE device_refresh_tokens SET used_at = $2, replaced_by = $3 WHERE id = $1',
					[id, now, nextRefreshId]
				);
				await client.query(
					`INSERT INTO device_access_tokens
				 (id, family_id, organization_id, project_id, scopes, token_hash,
				...
				return {
					...
					refreshExpiresIn: Math.max(
						1,
						Math.floor((row.family_expires_at.getTime() - now.getTime()) / 1_000)
					),
					accessExpiresIn: Math.max(
						1,
						Math.floor((accessExpiresAt.getTime() - now.getTime()) / 1_000)
					)
				};
			})
			.catch(throwDeviceRetentionCapacity);
		if ('error' in outcome && outcome.error !== undefined) {
			if (outcome.error === 'slow_down') {
				throw new DeviceAuthorizationError(
					'slow_down',
					'The refresh credential was rotated too recently.',
					429,
			...
			throw new DeviceAuthorizationError(
				outcome.error,
				outcome.error === 'refresh_reuse_detected'
					? 'Refresh-token reuse was detected; this login has been revoked.'
	...
	async revoke(
		...
		if (!id) return false;
		const now = this.now();
		const found = await this.database.query<{ token_hash: Buffer }>(
			`SELECT refresh_token.token_hash
			 FROM device_refresh_tokens refresh_token
			 JOIN device_refresh_families family ON family.id = refresh_token.family_id
			 WHERE refresh_token.id = $1
		...
		if (!equalDigest(found.rows[0]?.token_hash, refreshToken)) return false;
		return this.database.transaction(async (client) => {
			const locked = await client.query<RefreshRow>(
				`SELECT rt.id, rt.token_hash, rt.family_id, rt.generation, rt.used_at,
				        rt.expires_at AS token_expires_at, rf.expires_at AS family_expires_at,
	...
	async authenticateAccess(
		...
		const now = this.now();
		const found = await this.database.query<{ token_hash: Buffer }>(
			`SELECT access_token.token_hash
		...
		if (!equalDigest(found.rows[0]?.token_hash, raw)) return undefined;
		return this.database.transaction(async (client) => {
			const familyLock = await client.query<{ family_id: string }>(
				`SELECT family.id AS family_id
				 FROM device_access_tokens access_token
			...
			if (!familyLock.rows[0]) return undefined;
			const result = await client.query<AccessRow>(
				`SELECT access_token.id, access_token.token_hash, access_token.family_id,
				 ...
				 WHERE access_token.id = $1 AND family.id = $2 FOR UPDATE OF access_token`,
				[id, familyLock.rows[0].family_id]
			);
			const row = result.rows[0];
			if (
