...
const signatureValues = (header: string): { timestamp: number; signatures: string[] } => {
	let timestamp = 0;
...
export const verifyStripeSignature = (
	...
	const eventId =
		typeof candidate === 'object' && candidate !== null ? Reflect.get(candidate, 'id') : undefined;
	const eventType =
	...
	const eventCreatedAt =
		typeof eventCreated === 'number' ? new Date(eventCreated * 1_000) : undefined;
	if (
	...
	const object =
		typeof data === 'object' && data !== null ? Reflect.get(data, 'object') : undefined;
	...
	if (eventType === 'invoice.paid' || eventType === 'invoice.payment_failed') {
		...
		if (
			...
			throw new StripeWebhookRequestError(
				'invalid_stripe_event',
				'Stripe invoice event identity is malformed.'
			);
		}
	}
...
export class StripeWebhookService {
	...
	async process(event: StripeEvent, context: StripeProcessContext = {}): Promise<boolean> {
		return this.database.transaction(async (client) => {
			const inserted = await client.query(
				`INSERT INTO stripe_events (id, event_type, stripe_created_at, payload)
				 VALUES ($1, $2, to_timestamp($3), $4) ON CONFLICT (id) DO NOTHING`,
				[event.id, event.type, event.created, event]
			);
			if (!inserted.rowCount) return false;
			const customer = event.data.object.customer;
			const invoiceId = event.data.object.id;
			const targetDelinquent =
			...
			if (
				...
				const lookup = await client.query<{ organization_id: string }>(
					'SELECT organization_id FROM billing_accounts WHERE stripe_customer_id = $1',
					[customer]
				);
				...
				}
				{
					...
					if (organization.rows[0]) {
						const account = await client.query<{
							...
							`SELECT organization_id, delinquent_at FROM billing_accounts
							 WHERE organization_id = $1 AND stripe_customer_id = $2 FOR UPDATE`,
						...
						if (prior) {
							const invoice = await client.query<{
								...
								stripe_event_rank: number;
								stripe_event_id: string;
							}>(
								`SELECT failed, stripe_event_created_at, stripe_event_rank, stripe_event_id
								 FROM stripe_invoice_delinquency
								 WHERE organization_id = $1 AND stripe_invoice_id = $2 FOR UPDATE`,
							...
							if (!newer) {
								...
								await client.query(
									`INSERT INTO stripe_invoice_delinquency
									 ...
									 SET failed = EXCLUDED.failed,
									     ...
									     updated_at = now()`,
									[organizationId, invoiceId, targetDelinquent, event.created, eventRank, event.id]
								);
								...
								const aggregate = await client.query<{ delinquent: boolean }>(
									`SELECT EXISTS (
									...
									await client.query(
										`UPDATE billing_accounts
										 SET delinquent_at = CASE WHEN $3 THEN to_timestamp($4) ELSE NULL END,
										     updated_at = now()
										 WHERE organization_id = $1 AND stripe_customer_id = $2`,
										[organizationId, customer, delinquent, event.created]
									);
									await client.query(
										`INSERT INTO audit_events
								 (event_key, organization_id, actor_type, actor_id, action, outcome,
