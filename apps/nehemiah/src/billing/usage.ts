...
export class UsageLedger {
	...
	async append(event: UsageEvent): Promise<boolean> {
		if (!Number.isFinite(event.quantity) || event.quantity < 0) {
			throw new Error('usage quantity must be a non-negative finite number');
		}
		if (event.periodEnd < event.periodStart) throw new Error('usage period is inverted');
	...
	async recordMachineRuntime(input: {
		...
		if (input.end < input.start) throw new Error('usage period is inverted');
