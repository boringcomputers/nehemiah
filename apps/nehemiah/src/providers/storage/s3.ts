...
const templateObjectKey =
	/^organizations\/[A-Za-z0-9][A-Za-z0-9_-]{0,127}\/projects\/[A-Za-z0-9][A-Za-z0-9_-]{0,127}\/templates\/[a-z0-9][a-z0-9._-]{0,62}\/[A-Za-z0-9][A-Za-z0-9._+-]{0,63}\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/snapshot\.tar\.zst$/;
...
const validatedEndpoint = (value: string): URL => {
	let endpoint: URL;
	try {
...
const validateConfig = (config: S3TemplateObjectStoreConfig): URL => {
...
const validateObjectKey = (objectKey: string): void => {
...
const sha256Hex = (encoded: string | undefined): string => {
...
export class S3TemplateObjectStore implements TemplateObjectStore {
	...
	constructor(
		config: S3TemplateObjectStoreConfig,
		dependencies: S3TemplateObjectStoreDependencies = {}
	) {
	...
	createUploadGrant(
		...
		if (
			!templateChecksum.test(artifact.checksum) ||
			!Number.isSafeInteger(artifact.sizeBytes) ||
			artifact.sizeBytes < 1 ||
			artifact.sizeBytes > maximumSinglePutBytes
		) {
			throw new S3ObjectStoreContractError(
				'Template upload checksum or single-PUT size is invalid.'
			);
		}
	...
	async stat(
		...
		if (
			...
			throw new S3ObjectStoreContractError(
				'Stored template has invalid size, checksum type, or encryption metadata.'
			);
		}
		return {
	...
	#validateSignedUrl(
		...
		const exactObjectScope =
			(url.hostname === virtualHost && decodedPath === `/${objectKey}`) ||
		...
		if (
			...
			throw new S3ObjectStoreContractError(
				'S3 presigner returned a capability outside the requested object scope.'
			);
		}
	}
}
