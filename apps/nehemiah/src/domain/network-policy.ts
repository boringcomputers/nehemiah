...
const addCidr = (block: BlockList, cidr: string): void => {
...
export const isHardBlockedAddress = (
	ip: string,
	additionalBlockedCidrs: ReadonlyArray<string> = []
): boolean => {
	const version = isIP(ip);
	if (version === 0) return true;
	const block = new BlockList();
	for (const cidr of [...hardFloorCidrs, ...additionalBlockedCidrs]) addCidr(block, cidr);
...
const validHostnameRule = (hostname: string): boolean =>
	/^(?:\*\.)?(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/.test(hostname);

const hostnameMatches = (rule: string, hostname: string): boolean =>
	rule.startsWith('*.')
		? hostname.endsWith(rule.slice(1)) && hostname !== rule.slice(2)
		: hostname === rule;

interface LearnedAddress {
...
export class EgressPolicy {
	...
	readonly #learned = new Map<string, LearnedAddress>();

	constructor(
		declaration: NetworkPolicyDeclaration,
		private readonly additionalBlockedCidrs: ReadonlyArray<string> = []
	) {
		if (!declaration || typeof declaration !== 'object' || Array.isArray(declaration)) {
			throw new Error('network policy must be an object');
		}
		const unknownKeys = Object.keys(declaration).filter(
			(key) => !['mode', 'hostnames', 'cidrs'].includes(key)
		);
		if (unknownKeys.length > 0) throw new Error('network policy contains an unknown field');
		if (declaration.mode !== 'off' && declaration.mode !== 'allowlist') {
			throw new Error('network mode must be off or allowlist');
		}
		if (
			(declaration.hostnames !== undefined &&
	...
	allowsHostname(hostname: string): boolean {
		if (this.declaration.mode === 'off') return false;
	...
	learnDns(
		...
		const normalized = normalizeHostname(hostname);
		if (!this.allowsHostname(normalized)) throw new Error('hostname is not allowlisted');
		if (!addresses.length) throw new Error('DNS response has no addresses');
		...
		for (const address of addresses) {
			if (isIP(address) === 0) throw new Error('DNS response contains an invalid address');
			this.#learned.set(address, { hostname: normalized, expiresAt: now + ttl * 1_000 });
		}
	}

	allowsAddress(ip: string, now = Date.now()): boolean {
