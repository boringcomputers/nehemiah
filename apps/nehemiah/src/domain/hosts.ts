...
import {
	InvalidRuntimeCohort,
	parseRuntimeCohort,
	runtimeCohortWire,
	type RuntimeCohort
} from './runtime-cohort.js';
...
type HostHeartbeatOutcome = 'accepted' | 'rate_limited' | 'conflict';
...
interface HostLifecycleRow {
	readonly id: string;
	readonly desired_state: HostDesiredState;
	readonly credential_generation: number;
	readonly credential_status: HostCredentialStatus;
	readonly credential_rotated_at: Date;
...
const lifecycle = (row: HostLifecycleRow): HostLifecycle => ({
	id: row.id,
...
const checkedReason = (reason: string, fallback: string): string => {
...
const newHostCredential = async (): Promise<{ credential: string; credentialHash: string }> => {
...
const validateHostCredentials = (
	...
	if (
		...
		throw new HostEnrollmentError(
			...
			'host control, gateway, and enrollment credentials must be strong and distinct'
		);
	}
};

const enrollmentTokenHash = (token: string): Buffer =>
	createHash('sha256').update(token, 'utf8').digest();

const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

const validateHostIdentity = (
	input: Pick<
		RegisterHost,
		'providerId' | 'address' | 'totalVcpus' | 'totalMemoryMb' | 'totalDiskMb'
	>,
	allowedAddresses: BlockList
): void => {
	...
	if (
		...
		throw new HostEnrollmentError(
			'invalid_host_enrollment',
			'host address must be a literal IP in the managed overlay network'
		);
	}
};
...
const validateCohort = (
	...
	try {
		...
		throw new HostEnrollmentError(
			'invalid_host_enrollment',
			error instanceof InvalidRuntimeCohort ? error.message : 'runtime cohort is invalid'
		);
	}
};

export class HostService {
	readonly #allowedAddresses = new BlockList();

	constructor(
		private readonly database: Queryable,
		private readonly credentialCipher = new HostCredentialCipher(
	...
	async register(
		...
		const eligible = await this.database.query(
			`SELECT 1 FROM host_enrollment_grants
	...
	async issueEnrollment(input: IssueHostEnrollment): Promise<HostEnrollmentGrant> {
		validateHostIdentity(input, this.#allowedAddresses);
		const cohort = validateCohort(input.runtimeCohort, input.architecture);
		if (
			...
			throw new HostEnrollmentError(
				'invalid_host_enrollment',
				'ttlSeconds must be an integer between 60 and 1800'
			);
		}
		if (!uuid.test(input.issuedByOrganizationId) || !input.issuedByUserId.trim()) {
			throw new HostEnrollmentError(
				'invalid_host_enrollment',
				'an issuing organization and user are required'
			);
		}
		...
		if (!grant) {
			throw new HostEnrollmentError(
				'host_not_enrollable',
				'The provider host is already enrolled or is not eligible for enrollment.'
			);
		}
		return { id: grant.id, hostId: grant.host_id, token, expiresAt: grant.expires_at };
	}

	async revokeEnrollment(grantId: string): Promise<boolean> {
		const result = await this.database.query(
			`UPDATE host_enrollment_grants
			 SET revoked_at = statement_timestamp()
	...
	async gatewayCredential(hostId: string): Promise<string | undefined> {
		const result = await this.database.query<{ gateway_credential_ciphertext: string | null }>(
			`SELECT gateway_credential_ciphertext FROM hosts
	...
	async authenticate(hostId: string, credential: string): Promise<boolean> {
		return (await this.authenticateGeneration(hostId, credential)) !== undefined;
	}

	async authenticateGeneration(hostId: string, credential: string): Promise<number | undefined> {
		const result = await this.database.query<{
			credential_hash: string | null;
			credential_generation: number;
		}>(
			`SELECT credential_hash, credential_generation FROM hosts
			 WHERE id = $1 AND state <> 'stale'
			   AND desired_state IN ('active', 'draining')
			   AND credential_status = 'active'`,
			[hostId]
	...
	async heartbeat(
		...
		if (
			...
			throw new HostHeartbeatError(
				'invalid_host_heartbeat',
				'heartbeat capacity or daemon version is invalid'
			);
		}
		if (
			this.expectedDaemonVersion !== undefined &&
			heartbeat.daemonVersion !== this.expectedDaemonVersion
		) {
			throw new HostHeartbeatError(
				'invalid_host_heartbeat',
				'heartbeat daemon version is not approved for this control-plane release'
			);
		}
		let cohort: RuntimeCohort;
		try {
			cohort = parseRuntimeCohort(runtimeCohortWire(heartbeat.runtimeCohort));
		} catch (error) {
			throw new HostHeartbeatError(
				'invalid_host_heartbeat',
				error instanceof Error ? error.message : 'runtime cohort is invalid'
			);
		}
		const state: HostState = heartbeat.kvmAvailable ? heartbeat.state : 'unhealthy';
		const result = await this.database.query<{ result: HostHeartbeatOutcome | 'invalid' }>(
			`WITH current_host AS MATERIALIZED (
			 SELECT * FROM hosts
			 WHERE id = $1 AND state <> 'stale'
			   AND desired_state IN ('active', 'draining')
		...
		if (outcome === 'invalid') {
			throw new HostHeartbeatError(
				'invalid_host_heartbeat',
				'heartbeat capacity or runtime cohort does not match the enrolled host'
			);
		}
		if (outcome === 'rate_limited') {
			throw new HostHeartbeatError(
				'host_heartbeat_rate_limited',
				'heartbeat cadence exceeds the durable per-host limit',
				5
			);
		}
	...
	async markStale(staleAfterMs: number): Promise<ReadonlyArray<string>> {
		const result = await this.database.query<{ id: string }>(
			`UPDATE hosts SET state = 'stale', updated_at = now()
	...
	async setDraining(hostId: string, draining: boolean): Promise<boolean> {
		return Boolean(draining ? await this.drain(hostId) : await this.activate(hostId));
	}

	async isOperatorOrganization(organizationId: string): Promise<boolean> {
		const result = await this.database.query(
			'SELECT 1 FROM fleet_operator_organizations WHERE organization_id = $1',
			[organizationId]
		);
		return Boolean(result.rowCount);
	}
	...
	): Promise<HostLifecycle | undefined> {
		const result = await this.database.query<HostLifecycleRow>(
			`UPDATE hosts
			...
			[hostId, checkedReason(reason, 'operator requested drain')]
		);
	...
	async activate(
		...
		const result = await this.database.query<HostLifecycleRow>(
			`UPDATE hosts
			 ...
			 RETURNING ${lifecycleReturning}`,
			[hostId, checkedReason(reason, 'operator requested activation')]
		);
		return result.rows[0] ? lifecycle(result.rows[0]) : undefined;
	}
	...
	async quarantine(
		hostId: string,
		reason = 'operator quarantined host'
	): Promise<HostLifecycle | undefined> {
		const result = await this.database.query<HostLifecycleRow>(
			`UPDATE hosts
			 ...
			 RETURNING ${lifecycleReturning}`,
			[hostId, checkedReason(reason, 'operator quarantined host')]
		);
		return result.rows[0] ? lifecycle(result.rows[0]) : undefined;
	}
	...
	async revoke(
		...
		const result = await this.database.query<HostLifecycleRow>(
			`UPDATE hosts
			 ...
			 WHERE id = $1
			 RETURNING ${lifecycleReturning}`,
			[hostId, checkedReason(reason, 'operator revoked host')]
		);
		return result.rows[0] ? lifecycle(result.rows[0]) : undefined;
	}
	...
	async rotateCredentials(
		hostId: string,
		input: Pick<RegisterHost, 'controlToken' | 'gatewayToken'>,
		reason = 'operator rotated host credentials'
	): Promise<(HostLifecycle & { readonly credential: string }) | undefined> {
