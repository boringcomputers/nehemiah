...
export const databaseRoleFromUrl = (raw: string): string => {
	let parsed: URL;
	try {
		parsed = new URL(raw);
	} catch {
...
const quoteRole = (role: string): string => {
	if (!rolePattern.test(role)) throw new Error('invalid PostgreSQL runtime role name');
	return `"${role}"`;
};
 ...
 */
export const applyRuntimeRolePrivileges = async (
	database: Queryable,
	runtimeRole: string
): Promise<void> => {
	...
	await database.query(
		`REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON schema_migrations FROM ${role}`
	);
	for (const ledger of [
		'machine_events',
		'usage_events',
		...
		await database.query(
			`REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON ${ledger} FROM ${role}`
		);
	}
	await database.query(
...
export const assertSeparatedDatabaseRoles = async (
	database: Queryable,
	runtimeRole: string
): Promise<void> => {
	const result = await database.query<{ current_user: string; database_owner: string }>(`
		SELECT current_user,
	...
	if (!row) throw new Error('could not inspect PostgreSQL role ownership');
