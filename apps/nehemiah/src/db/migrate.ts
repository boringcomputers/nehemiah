...
const run = async (): Promise<void> => {
	const production =
		process.env.NEHEMIAH_ENV === 'production' || process.env.NODE_ENV === 'production';
	const runtimeDatabaseUrl =
		process.env.DATABASE_URL?.trim() ||
		(production ? '' : 'postgres://postgres:postgres@127.0.0.1:5432/nehemiah');
	if (!runtimeDatabaseUrl) throw new Error('DATABASE_URL is required in production');
	...
	if (!directory) throw new Error('database migration directory is missing');
	try {
		...
		const migrated = await database.withAdvisoryLock(migrationLockKey, async (client) => {
			...
			const unknown = await client.query<{ name: string }>(
				`SELECT name FROM schema_migrations
				 WHERE NOT (name = ANY($1::text[])) ORDER BY name`,
				[names]
			);
			if (unknown.rows.length) {
				throw new Error(
					`database contains migrations absent from this build: ${unknown.rows
						.map(({ name }) => name)
						.join(', ')}`
				);
			}
