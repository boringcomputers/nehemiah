...
export const volumeDeletionRetryDelayMs = (attemptCount: number): number =>
	Math.min(60 * 60 * 1_000, 1_000 * 2 ** Math.min(Math.max(attemptCount - 1, 0), 12));
...
export class VolumeDeletionWorker {
	...
	async run(limit = volumeDeletionBatchSize): Promise<VolumeDeletionRun> {
		...
		if (deliveryErrors.length) {
			throw new AggregateError(
				deliveryErrors,
				'one or more volume deletion jobs could not advance'
			);
		}
		return {
			expiredEnqueued,
			...
			scheduled: results.filter(
				(result) => result.status === 'fulfilled' && result.value === 'scheduled'
			).length,
	...
	async #deliver(job: VolumeDeletionJob): Promise<'scheduled' | 'deferred'> {
		const deliveryStartedAt = this.now();
		if (
			this.storage.deletionMode === 'at-retention-boundary' &&
			...
			if (!deferred) throw new Error('volume retention claim disappeared before deferral');
			return 'deferred';
		}
		try {
			...
			if (!deferred) throw new Error('volume deletion claim disappeared before deferral');
			return 'deferred';
		}
	}
}
