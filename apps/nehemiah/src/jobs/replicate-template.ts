...
export class PostgresTemplateReplicaRepository implements TemplateReplicaRepository {
	...
	async claim(): Promise<TemplateReplicaWork | undefined> {
		const result = await this.database.query<ReplicaRow>(
	...
	async markReady(templateId: string, hostId: string, verifiedChecksum: string): Promise<boolean> {
		const result = await this.database.query(
			`UPDATE template_replicas tr SET state = 'ready', progress = 100,
			 ...
			 WHERE tr.template_id = $1 AND tr.host_id = $2 AND tr.state = 'pulling'
			   ...
			   AND t.checksum = $3`,
			[templateId, hostId, verifiedChecksum]
		);
	...
	async markFailed(templateId: string, hostId: string, reason: string): Promise<void> {
		await this.database.query(
			`UPDATE template_replicas SET state = 'failed', progress = 0,
			        verified_checksum = NULL, error = $3, updated_at = now()
			 WHERE template_id = $1 AND host_id = $2 AND state = 'pulling'`,
			[templateId, hostId, reason.slice(0, 256)]
		);
	}
}

export type ReplicationRunResult =
	| { readonly state: 'idle'; readonly enqueued: number }
	| {
...
const validWork = (work: TemplateReplicaWork): boolean => {
	...
	return (
		[work.templateId, work.organizationId, work.projectId, work.name, work.version].every((value) =>
...
export class TemplateReplicationJob {
	constructor(
		private readonly repository: TemplateReplicaRepository,
		private readonly storage?: TemplateObjectStore,
		private readonly activator?: TemplateActivator,
		private readonly now: () => Date = () => new Date()
	) {}

	async runOnce(): Promise<ReplicationRunResult> {
		if (!this.storage || !this.activator) {
			throw new TemplateInfrastructureUnavailable(
				'Durable template replication is not configured on this control plane.'
			);
		}
		const enqueued = await this.repository.enqueueMissing();
		const work = await this.repository.claim();
		if (!work) return { state: 'idle', enqueued };
		...
		try {
			...
			const now = this.now();
			const download = await this.storage.createDownloadGrant(
				work.objectKey,
			...
			if (activated.checksum !== work.checksum || activated.sizeBytes !== work.sizeBytes) {
				throw new TemplateIntegrityError(
					'The host activation checksum or size did not match the immutable manifest.'
				);
			}
			...
			const reason =
				error instanceof TemplateIntegrityError ? error.code : 'template_activation_failed';
