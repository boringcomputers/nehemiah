...
import type { HostClient, HostMachine } from '../clients/nehemiahd.js';
import { HostForkContractError, HostRequestError } from '../clients/nehemiahd.js';
import type { Database, Queryable } from '../db/client.js';
import { PostgresMachineRepository, type ForkOperation } from '../domain/machines.js';
...
type ReconcileRow = {
	...
	runtime_cohort_id: string;
	source_sha256: string;
	lease_id: string;
	lease_generation: string;
	state: 'starting' | 'stopping' | 'running';
	...
	startup_deadline_reached: boolean;
...
type UsageOutboxRow = {
	...
	period_end: Date;
	final: boolean;
	source: 'control-plane' | 'host' | 'reconciler';
	attempts: number;
	claim_token: string;
};
...
const forEachConcurrent = async <A>(
...
export class MachineReconciler {
	private readonly machines: PostgresMachineRepository;

	constructor(
		private readonly database: Database,
		private readonly host: HostClient,
		private readonly usage?: UsageLedger,
		private readonly metering?: AuthoritativeMetering
	) {
	...
	private async runLocked(): Promise<void> {
			...
			try {
				...
				if (operation.reconcileClaimToken) {
					await this.machines
						...
						.catch((error) =>
							log('warn', 'machine fork reconciliation claim release failed', {
								forkOperationId: operation.id,
								error: String(error)
							})
						);
		...
		await forEachConcurrent(machines, reconcileConcurrency, async (machine) => {
			...
			try {
				...
				await this.releaseReconcileClaim(machine, failed).catch((error) =>
					log('warn', 'machine reconciliation claim release failed', {
						machineId: machine.id,
						error: String(error)
					})
				);
	...
	private async closeUnfinalizedAuthoritativeTerminals(): Promise<void> {
		if (!this.metering) return;
		const result = await this.database.query<{ id: string; lease_id: string }>(
			`SELECT machine.id, machine.lease_id
	...
	private async reconcileFork(operation: ForkOperation): Promise<void> {
		let active = operation;
		if (active.cleanupRequested) {
			active = await this.machines.requestForkCleanup(
				active.id,
			...
			if (active.state !== 'pending' || !active.cleanupClaimToken) return;
		}
		...
		try {
			...
			if (error instanceof HostRequestError && error.code === 'fork_batch_cleaned') {
				await this.finishForkCleanup(
					active,
					502,
					'fork_batch_failed',
					'The host removed every reserved child after the fork could not complete.',
					[],
					true
				);
				return;
			...
			if (error instanceof HostForkContractError) {
				await this.finishForkCleanup(
					...
					502,
					'fork_batch_failed',
					'The host never returned a valid, complete fork batch.',
					error.observed
				);
				return;
			}
			if (error instanceof HostRequestError && error.ambiguous) {
				...
				await this.finishForkCleanup(
					...
					502,
					'fork_batch_failed',
					'The host result is ambiguous; the fork remains hidden until cleanup is confirmed.',
					[]
				);
				return;
			}
			...
			if (error instanceof HostRequestError && !error.ambiguous) {
				...
				await this.machines.failFork(
					active.id,
					sourceRejected ? 422 : 502,
					sourceRejected ? 'machine_not_forkable' : 'host_request_failed',
					sourceRejected
		...
		try {
			...
			if (!(error instanceof HostRequestError && error.ambiguous)) throw error;
			await this.finishForkCleanup(
				active,
				502,
				'fork_batch_failed',
				'The host fork batch did not match the durable child reservation.',
				observed
			);
		}
	}
	...
	private async finishForkCleanup(
		...
		if (cleanup.state !== 'pending' || !cleanup.cleanupClaimToken) return;
	...
	private async claimReconcileBatch(): Promise<ReconcileRow[]> {
		...
		return this.database.transaction(async (client) => {
			const result = await client.query<ReconcileRow>(
				`WITH candidates AS (
				 ...
				 ORDER BY m.reconcile_after, m.created_at, m.id`,
				[reconcileBatchSize, claimToken, reconcileClaimSeconds]
			);
			return result.rows;
		});
	}

	private async releaseReconcileClaim(machine: ReconcileRow, failed: boolean): Promise<void> {
		await this.database.query(
			`UPDATE machines SET reconcile_claim_token = NULL, reconcile_claimed_until = NULL,
			 reconcile_after = CASE WHEN $4 THEN
			   GREATEST(reconcile_after, now() + interval '30 seconds') ELSE reconcile_after END
			 WHERE id = $1 AND lease_id = $2 AND reconcile_claim_token = $3`,
			[machine.id, machine.lease_id, machine.reconcile_claim_token, failed]
		);
	}

	private async reconcile(machine: ReconcileRow): Promise<void> {
		let current = machine;
		let hostMachineId = machine.host_machine_id;
		let recovered: HostMachine | undefined;
		if (!hostMachineId) {
			...
			const persisted = await this.database.query<{ state: ReconcileRow['state'] }>(
		...
		if (
			...
			throw new Error(
				'host observation does not match the current machine lease, resources, and network policy'
			);
		}
		if (observed && hostTerminalStates.has(observed.status)) {
			await this.finishAndRelease(
				current,
				observed.status === 'stopped' ? 'stopped' : 'failed',
				`host reported ${observed.status}`
			);
			return;
		}
		if (observed && current.state === 'running') {
			await this.database.query(
				`UPDATE machines SET ready = $2,
		...
		if (observed && current.state === 'starting') {
			...
			await this.database.transaction(async (client) => {
				const result = await client.query(
					`UPDATE machines SET state = $2, ready = $3,
	...
	private async markLostOnStaleHosts(): Promise<void> {
		const lost = await this.database.transaction(async (client) => {
			...
			return client.query<ReconcileRow>(
				`UPDATE machines m SET state = 'lost', state_reason = 'host heartbeat expired',
			 stopped_at = COALESCE(
	...
	private async finishAndRelease(
		machine: ReconcileRow,
		state: 'stopped' | 'failed' | 'lost',
		reason: string
	): Promise<void> {
		const lossCutoff =
			state === 'lost' && this.metering
				? await this.metering.closeHostLoss(machine.id, machine.lease_id)
				: undefined;
		const changed = await this.database.transaction(async (client) => {
			const result = await client.query<{ old_state: ReconcileRow['state'] }>(
				`WITH prior AS (
				 ...
				 ) SELECT old_state FROM updated`,
				[machine.id, state, reason, machine.lease_id, lossCutoff ?? null]
			);
	...
	private async releaseLeakedReservations(): Promise<void> {
		const result = await this.database.query<ReconcileRow>(
			`SELECT m.id, m.organization_id, m.project_id, m.host_id,
			 host(h.address) AS host_address, m.host_machine_id, m.lease_id,
			 m.lease_generation::text,
			 'running'::text AS state, m.template_name, m.template_id,
		...
		for (const machine of result.rows) await this.release(machine);
	}

	private async release(machine: ReconcileRow): Promise<void> {
		await this.database.transaction(async (client) => {
			...
			await client.query(
				`UPDATE hosts SET reserved_vcpus = GREATEST(0, reserved_vcpus - $2),
				 ...
				 WHERE id = $1`,
				[machine.host_id, machine.vcpus, machine.memory_mb, machine.disk_mb]
			);
		});
	}
	...
	private async event(
		...
		await database.query(
			`INSERT INTO machine_events
			 (event_key, machine_id, organization_id, from_state, to_state, reason, observed_by)
			 VALUES ($1, $2, $3, $4, $5, $6, 'reconciler') ON CONFLICT DO NOTHING`,
			[`${machine.id}:reconciled:${to}`, machine.id, machine.organization_id, from, to, reason]
		);
	}

	private async enqueueUsageIntervals(excludeAuthoritativeLeases = false): Promise<void> {
		await this.database.transaction(async (client) => {
			...
			const terminal = await client.query<{
				...
				`SELECT id, organization_id, project_id, vcpus, memory_mb,
					 ...
					 ORDER BY stopped_at, id FOR UPDATE SKIP LOCKED LIMIT 100`,
				[excludeAuthoritativeLeases]
			);
			for (const row of terminal.rows) {
				if (
					...
					await client.query(
						`UPDATE machines SET usage_quarantined_at = now(),
	...
	private async flushUsageOutbox(): Promise<void> {
		const usage = this.usage;
		if (!usage) return;
		const claimToken = randomUUID();
		const pending = await this.database.transaction(async (client) => {
			return client.query<UsageOutboxRow>(
				`WITH candidates AS (
				 ...
				 FROM claimed ORDER BY next_attempt_at, created_at, event_key`,
				[reconcileBatchSize, claimToken, usageClaimSeconds]
			);
		});
			...
			try {
				...
				await this.database.transaction(async (client) => {
					const processed = await client.query(
						`UPDATE usage_outbox SET processed_at = now(), attempts = attempts + 1,
						 last_error = NULL, claim_token = NULL, claimed_until = NULL
						 WHERE event_key = $1 AND claim_token = $2 AND processed_at IS NULL
						 RETURNING machine_id`,
						[event.event_key, event.claim_token]
					);
					if (processed.rowCount) {
						await client.query(
							`UPDATE machines m SET usage_finalized_at = now()
				...
				await this.database.query(
					`UPDATE usage_outbox SET attempts = attempts + 1, last_error = $3,
					 next_attempt_at = now() + ($4 * interval '1 second'),
					 claim_token = NULL, claimed_until = NULL
					 WHERE event_key = $1 AND claim_token = $2 AND processed_at IS NULL`,
					[event.event_key, event.claim_token, String(error).slice(0, 1_024), delaySeconds]
				);
			}
		});
	}
