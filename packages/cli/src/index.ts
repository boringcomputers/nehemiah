...
export class DeviceLoginUnavailable extends Error {
	...
	constructor(status?: number) {
		super(
			'This endpoint does not currently offer device login. Configure NEHEMIAH_API_KEY or run `bc config set-key`.'
		);
		this.status = status;
	}
}
...
export class CredentialOriginError extends Error {
	constructor(
		readonly code:
			'credential_origin_required' | 'credential_origin_mismatch' | 'unsafe_credential_origin',
		message: string
	) {
		super(message);
	}
...
export async function runCli(
	...
	const io = dependencies.io ?? {
		...
		error: (text: string) => stderr.write(`${text}\n`)
	};
	...
	try {
		...
		const machineArgs =
			noun === 'machine' || noun === 'machines'
				? rest
				: ([rawCommand, ...rest].filter(Boolean) as string[]);
		if (machineCommand === 'tty') {
			const id = machineArgs[0];
			if (!id) throw new UsageError('Missing machine id.');
			const client =
				dependencies.client ??
				make(await authenticatedClientOptions(env, configFile, parsed.flags, dependencies));
...
export function clientOptions(
	...
	const cloudCredential =
		explicitCredential ?? (storedCredentialIsBound ? storedCredential : undefined);
...
const credentialStore = (dependencies: CliDependencies): CredentialStore =>
	dependencies.credentialStore ?? new SystemCredentialStore();

const selectedBaseUrl = (
	env: NodeJS.ProcessEnv,
	config: CliConfig,
	flags: Readonly<Record<string, string | boolean>>
): string => flagString(flags, 'url') ?? env.NEHEMIAH_URL ?? config.baseUrl ?? CLOUD_BASE_URL;

const safeCredentialOrigin = (baseUrl: string): string => {
	const origin = canonicalCredentialOrigin(baseUrl);
	if (!origin) {
		throw new CredentialOriginError(
			'unsafe_credential_origin',
			'Stored credentials require an HTTPS API origin (loopback HTTP is allowed for local development).'
		);
	}
	return origin;
};
...
async function requireCredentialBinding(input: {
	...
	if (configuredOrigin !== credentialOrigin || selectedOrigin !== credentialOrigin) {
		throw new CredentialOriginError(
			...
			'The selected API origin does not match the origin bound to this stored credential. Remove URL overrides or authenticate explicitly for the selected origin.'
		);
	}
...
const requestedScopes = (
	...
	if (value === undefined) return cliDeviceScopes;
	const scopes = [
		...new Set(
			value
				.split(',')
	...
	const allowed = new Set<string>([...cliDeviceScopes, 'billing:read']);
...
const clock = (dependencies: CliDependencies): number => {
...
const refreshFromStoredCredential = (secret: string): string | undefined => {
...
async function authenticatedClientOptions(
	env: NodeJS.ProcessEnv,
	configFile: string,
	flags: Readonly<Record<string, string | boolean>>,
	dependencies: CliDependencies
): Promise<NehemiahClientOptions> {
	const config = await readConfig(configFile);
	const options = clientOptions(env, config, flags);
	if (env.NEHEMIAH_API_KEY) return options;
	...
	if (options.apiKey || config.authMode !== 'device' || !config.credentialAccount) return options;
	...
	return withCredentialLock(configFile, account, async () => {
		...
		if (currentConfig.authMode !== 'device' || currentConfig.credentialAccount !== account) {
			throw new DeviceAuthError(
				...
				'This device login changed in another process. Retry the command.'
			);
		}
		...
		if (!secret) {
			throw new DeviceAuthError(
				'device_login_required',
				'This device login is missing from the operating-system credential store. Run `bc login` again.'
			);
		}
		const decoded = decodeDeviceCredential(secret);
		if (decoded?.kind === 'legacy') {
			throw new DeviceAuthError(
				'device_session_upgrade_required',
				'This login predates safe access-token caching. Run `bc login` again.'
			);
		}
		if (
			decoded?.kind !== 'session' ||
			...
			throw new DeviceAuthError(
				...
				'The stored device session is invalid. Run `bc login` again.'
			);
		}
		...
		if (decoded.session.refreshExpiresAt <= now) {
			...
			throw new DeviceAuthError(
				'device_login_required',
				'This device login expired. Run `bc login` again.'
			);
		}
		...
		try {
			token = await refreshDeviceToken(
				currentBinding.boundBaseUrl,
				decoded.session.refreshToken,
...
async function runDeviceLogin(input: {
	...
	if (config.authMode === 'device' && config.credentialAccount) {
		try {
			...
			if (
				!(error instanceof CredentialOriginError) ||
	...
	emit(
		display,
		`Open ${code.verification_uri_complete} (code ${code.user_code}) to approve this login.`,
		input.io,
		input.json
	);
	if (!input.json && input.flags['no-browser'] !== true) {
		await (
			input.dependencies.openBrowser
				? input.dependencies.openBrowser(code.verification_uri_complete)
				: openVerificationPage(code.verification_uri_complete, baseUrl)
		).catch(() => undefined);
	}
	...
	try {
		await withCredentialLock(input.configFile, account, async () => {
			...
			try {
				const previous =
					config.authMode === 'device' && config.credentialAccount && previousBaseUrl
				...
				if (previous && previousBaseUrl) {
					...
					if (!previousRefresh) {
						throw new DeviceAuthError(
							'device_credential_invalid',
							'The previous device session is invalid and could not be revoked.'
						);
					}
				...
				await revokeDeviceToken(baseUrl, token.refresh_token, fetchImplementation).catch(
					() => undefined
				);
				if (storedNewSession) await store.delete(account).catch(() => undefined);
				throw error;
			}
		});
	} catch (error) {
	...
	emit(
		...
		`Signed in for project ${token.project_id}. The bounded device session is in your OS credential store.`,
		input.io,
		input.json
	);
	return 0;
...
async function runDeviceLogout(input: {
	...
	await withCredentialLock(input.configFile, account, async () => {
		...
		if (currentConfig.authMode !== 'device' || currentConfig.credentialAccount !== account) {
			throw new DeviceAuthError(
				'device_login_required',
				'This device login changed in another process. Retry logout.'
			);
		}
		const binding = await requireCredentialBinding({
			kind: 'device',
		...
		if (secret) {
			...
			if (!refresh) {
				throw new DeviceAuthError(
					'device_credential_invalid',
					'The stored device session is invalid and could not be revoked.'
				);
			}
			await revokeDeviceToken(
				binding.boundBaseUrl,
				refresh,
				input.dependencies.fetch ?? globalThis.fetch
			);
		}
		await store.delete(account);
		await writeConfig(
...
async function runConfig(
	...
	if (command === 'set-key') {
		...
		if (!key) throw new UsageError('Missing API key. Pass it as an argument or --api-key.');
		...
		if (
			...
			throw new UsageError(
				'This saved API key has no trustworthy origin. Re-run `bc config set-key KEY --url URL` with the endpoint explicitly selected.'
			);
		}
	...
	if (command === 'set') {
		...
		if (requestedUrl !== undefined) safeCredentialOrigin(requestedUrl);
		if (
			...
			throw new UsageError(
				'Cannot change --url while a stored credential is active. For an API key, run `bc config set-key KEY --url URL`; for device login, run `bc logout` at its bound endpoint and then `bc login --url URL`.'
			);
		}
		...
		const next: CliConfig = {
			...
			...(targetValue ? { target: targetValue as TargetMode } : {})
		};
...
async function runTty(client: NehemiahClient, id: string): Promise<void> {
			...
			Effect.gen(function* () {
				...
				const handler = (chunk: Buffer | string) => {
					void Effect.runPromise(
						channel.send(typeof chunk === 'string' ? chunk : new Uint8Array(chunk))
					).catch(() => undefined);
				};
				...
				yield* Stream.runForEach(channel.output, (bytes) =>
					Effect.sync(() => stdout.write(Buffer.from(bytes)))
				);
			})
		)
	);
...
function emit(value: unknown, human: string, io: CliIo, json: boolean): void {
...
function formatError(error: unknown): FormattedError {
	...
	if (error instanceof RequestError) {
		return {
			code: 'request_error',
			message: `Request failed: ${error.method} ${error.path}.${
...
function validTarget(value: unknown): value is TargetMode {
