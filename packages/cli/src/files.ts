...
const absolutePath = (value: string): string => {
	if (!value || value.includes('\0')) throw new UnsafeLocalPath('A valid local path is required.');
...
export async function writePrivateAtomic(path: string, data: Uint8Array): Promise<string> {
