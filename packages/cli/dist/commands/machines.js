...
export async function runMachineCommand(client, command, args) {
    switch (command) {
        case 'create': {
            const hasSource = args.flags.template !== undefined ||
            ...
            if (allowedHostnames.length > 0 || allowedCidrs.length > 0) {
                throw new UsageError('--allow-hostnames and --allow-cidrs are unavailable; managed cloud is network-off and public local/self-hosted routes use only the legacy --net switch.');
            }
            const options = {
                ...
                ...enumFlag(args.flags, 'size', ['small', 'medium', 'large']),
        ...
        case 'list': {
            const page = await run(client.listMachinesPage({
                ...optionalString(args.flags, 'project'),
        ...
        case 'exec': {
            ...
            const result = await run(client.exec(id, commandText, {
        ...
        case 'download': {
            ...
            const downloaded = await run(client.downloadFile(id, remotePath, fileTransferOptions(args.flags)));
        ...
        case 'stop': {
            const id = required(args.positionals[0], 'machine id');
            await run(client.destroyMachine(id, {
                ...renamedString(args.flags, 'idempotency-key', 'idempotencyKey')
            }));
        ...
        case 'fork': {
            ...
            try {
                const machines = await run(client.branchMachines(id, count, {
                    ...renamedString(args.flags, 'idempotency-key', 'idempotencyKey')
                }));
                return {
                    value: { machines },
                ...
                if (!(error instanceof ForkPending))
                    throw error;
                const operation = {
...
function renamedString(flags, name, property) {
    const value = stringFlag(flags, name);
...
function renamedNumber(flags, name, property) {
    const value = numberFlag(flags, name);
    return value === undefined ? {} : { [property]: value };
}
function enumFlag(flags, name, values) {
    const value = stringFlag(flags, name);
    if (value === undefined)
        return {};
...
function fileTransferOptions(flags) {
    return {
